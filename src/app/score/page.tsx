...
const RARITY_CONFIG: Record<string, { label: string; color: string; glow: string; badge: string }> =
  {
    common: {
      ...
      badge: 'text-text-muted bg-bg-elevated',
    },
    rare: {
      label: 'Raro',
      color: '#3B82F6',
    ...
    legendary: {
      ...
      badge: 'text-brand-gold bg-brand-gold/10',
    },
  };
    ...
    <AppShell>
      <div className="mx-auto max-w-5xl space-y-8 p-4 md:p-8">
        ...
        >
          ...
          <div className="relative z-10 flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-text-secondary">Sua evolução em números.</p>
            </div>
        ...
        >
          {/* Decorative glow */}
          <div
            className="pointer-events-none absolute -right-8 -top-8 h-48 w-48 rounded-full opacity-30 blur-[60px]"
          ...
          <div className="flex flex-col items-center gap-8 md:flex-row">
            ...
            <div className="relative shrink-0">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                <div
                  className="heading-display text-lg leading-tight"
            ...
            <div className="flex-1 text-center md:text-left">
              <div className="mb-1 text-sm uppercase tracking-widest text-text-muted">
                Nível {profile.level} de 8
              </div>
              <div className="heading-display gradient-text mb-3 text-5xl leading-none md:text-6xl">
                {levelInfo.title}
              </div>
                ...
                <div className="max-w-md space-y-2">
                  <div className="flex justify-between text-xs text-text-muted">
                    ...
                    <span style={{ color: levelColor }}>
                      {(progress.needed - progress.current).toLocaleString('pt-BR')} XP até nível{' '}
                      {profile.level + 1}
                    </span>
                  </div>
            ...
            <div className="shrink-0 text-center md:text-right">
              ...
              <div className="text-xs uppercase tracking-widest text-text-muted">XP Total</div>
            </div>
          </div>
        </div>
        ...
        {unlockedAchievements.length > 0 && (
          <section>
            <div className="mb-4 flex items-center gap-3">
              ...
              <span className="rounded-full border border-brand-gold/20 bg-brand-gold/20 px-3 py-0.5 text-sm font-bold text-brand-gold">
            ...
            <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
              {unlockedAchievements.map((a) => {
                ...
                return (
                  <div
                    key={a.id}
                    className="relative overflow-hidden rounded-2xl p-4 text-center transition-transform hover:scale-[1.02]"
                    style={{
                  ...
                  >
                    ...
                    <div
                      ...
                      style={{
                        background: `radial-gradient(circle at 50% 30%, ${cfg.color} 0%, transparent 70%)`,
                      }}
                    />
                    ...
                    <div className="absolute right-2 top-2">
                      <span
                        className={`rounded-full px-1.5 py-0.5 text-[9px] font-bold uppercase ${cfg.badge}`}
                      >
                        {cfg.label}
                      </span>
                    </div>

                    <div className="relative z-10 mb-2 mt-2 text-4xl">{a.icon}</div>
                    <div className="relative z-10 text-sm font-bold">{a.name}</div>
                    <div className="relative z-10 mt-1 line-clamp-2 text-xs text-text-muted">
                      {a.description}
                    </div>
                    ...
                    {unlockedAt && (
                      <div className="relative z-10 mt-1.5 text-[10px] text-text-muted">
        ...
        {lockedAchievements.length > 0 && (
          <section>
            <div className="mb-4 flex items-center gap-3">
              ...
              <span className="rounded-full bg-bg-elevated px-3 py-0.5 text-sm text-text-muted">
            ...
            <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
              {lockedAchievements.map((a) => {
                ...
                return (
                  <div
                    key={a.id}
                    className="relative overflow-hidden rounded-2xl p-4 text-center opacity-45 transition-opacity hover:opacity-60"
                    style={{
                  ...
                  >
                    ...
                    <div className="mb-2 mt-2 text-4xl grayscale">{a.icon}</div>
                    <div className="text-sm font-bold text-text-secondary">{a.name}</div>
                    <div className="mt-1 line-clamp-2 text-xs text-text-muted">{a.description}</div>
                    <div className="mt-2">
                      <span
                        className={`rounded-full px-1.5 py-0.5 text-[10px] font-bold uppercase ${cfg.badge}`}
                      >
        ...
        {allAchievements.length === 0 && (
          ...
          >
            ...
            <p className="mt-1 text-sm text-text-muted">
              Continue usando o Ascendia para desbloquear conquistas!
            </p>
          </div>
        )}
      </div>
...
function StatCard({
  ...
  return (
    ...
    >
      ...
      <div className="heading-display relative z-10 text-3xl leading-tight">
        {prefix}
        {value}
      </div>
      {sub && <div className="relative z-10 mt-0.5 text-xs text-text-secondary">{sub}</div>}
    </div>
  );
}
