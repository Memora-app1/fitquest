    ...
    >
      ...
      <div className="relative z-10 space-y-4">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="h-6 w-12 shrink-0 rounded-full" style={{ background: `${accent}12` }} />
        </div>

        {/* Amounts */}
        <div className="grid grid-cols-3 gap-3">
          {[
            { label: 'Atual', big: true },
            { label: 'Meta', big: false },
            { label: 'Faltam', big: false },
          ].map(({ label, big }, i) => (
    ...
    <div className="mx-auto max-w-4xl space-y-6 p-4 md:p-8">
      ...
      >
        <div className="grid grid-cols-3 gap-4 text-center">
          {[
            { accent: '#00FF88', label: 'Ativas' },
            { accent: '#F5C842', label: 'Progresso médio' },
            { accent: '#3B82F6', label: 'Concluídas' },
          ].map(({ accent }, i) => (
            <div key={i} className="space-y-1">
              <div className="mx-auto h-7 w-12 rounded-lg" style={{ background: `${accent}12` }} />
              <div
