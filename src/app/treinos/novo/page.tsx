  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok || !data.workoutId) {
        setError(data.error === 'unauthorized' ? 'Sessão expirada. Faça login novamente.' : 'Erro ao salvar treino. Tente novamente.')
        setLoading(false)
        return
      }
      ...
      <main className="min-h-screen flex items-center justify-center p-6">
        <div className="card-glow w-full max-w-md p-10 text-center space-y-6 animate-slide-up">
          ...
          <h2 className="heading-display text-4xl gradient-text">
            {success.isPR ? 'Novo Record!' : 'Treino Feito!'}
          </h2>
  ...
  return (
    <main className="min-h-screen p-4 md:p-8">
      <div className="max-w-2xl mx-auto space-y-6">
        <Link href="/treinos" className="flex items-center gap-2 text-text-secondary hover:text-white transition-colors">
          <ArrowLeft size={18} />
          Voltar
        </Link>
