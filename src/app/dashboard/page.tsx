...
export default async function DashboardPage({
  ...
  const [
    ...
    supabase
      .from('profiles')
      .select(
        'name, xp_total, level, streak_current, streak_longest, subscription_status, trial_end, perfect_days, streak_freezes, login_streak, last_login_date, recovery_week_active, recovery_week_used_month'
      )
      .eq('id', user.id)
      .single(),
    supabase
  ...
  const totalScore = Math.round(
    habitsScore * 0.4 + streakScore * 0.25 + xpScore * 0.2 + taskScore * 0.15
  );
  ...
  return (
    <AppShell>
      <div className="mx-auto max-w-6xl space-y-6 p-4 md:p-8">
        ...
        {welcome === '1' && (
          <div
            className="relative flex animate-bounce-in items-center gap-4 overflow-hidden rounded-2xl p-5 md:p-6"
            style={{
              background:
                'linear-gradient(135deg, rgba(124,58,237,0.18) 0%, rgba(13,24,41,0.98) 55%, rgba(255,77,0,0.10) 100%)',
          ...
          >
            ...
            />
            <div className="relative z-10 animate-bounce text-4xl">🚀</div>
            ...
            <div
              className="relative z-10 hidden shrink-0 rounded-xl px-4 py-2 text-xs font-black sm:block"
              style={{
        ...
        {checkout === 'success' && (
          <div className="flex items-center gap-3 rounded-xl border border-brand-green/30 bg-brand-green/10 p-4">
            <span className="text-2xl">🎉</span>
            <div>
              <p className="font-semibold text-brand-green">Assinatura ativada com sucesso!</p>
              <p className="text-sm text-text-secondary">
                Bem-vindo ao Ascendia Premium. Curta todos os recursos.
              </p>
            </div>
          </div>
        )}
        ...
        {trialDaysLeft !== null && (
          ...
          >
            ...
            <Link
              href="/planos"
              className="shrink-0 rounded-xl px-4 py-2 text-sm font-bold transition-all hover:scale-105"
              style={{
        ...
        <header
          className="px-1 pb-0 pt-1 md:relative md:overflow-hidden md:rounded-2xl md:p-6"
          style={
            {
              // desktop only
            }
        ...
        >
          ...
          <div
            className="pointer-events-none absolute -right-8 -top-8 hidden h-40 w-40 rounded-full md:block"
            style={{
          ...
          <div className="relative z-10 flex items-center justify-between gap-3">
            ...
            <div className="min-w-0">
              <p className="text-xs text-text-muted md:text-sm">{getGreeting()}</p>
              <h1 className="truncate text-xl font-bold leading-tight md:text-4xl">
        ...
        {/* Guild do usuário */}
        <Suspense fallback={<div className="shimmer h-32 rounded-2xl" />}>
        ...
        {/* Season pass */}
        <Suspense fallback={null}>
          <SeasonPassWidget userId={user.id} isPaid={isPaid} />
        </Suspense>
