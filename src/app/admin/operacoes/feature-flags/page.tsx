...
export default async function FeatureFlagsPage() {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-5">
      <div>
        <h1 className="text-2xl font-black" style={{ color: '#fff' }}>Feature Flags</h1>
      ...
      <div className="space-y-3">
        {(flags ?? []).map(flag => (
          <FeatureFlagToggle
            key={flag.id}
            flag={flag}
            canEdit={hasMinRole(session, 'admin')}
          />
        ))}
        {(!flags || flags.length === 0) && (
          <p className="text-sm" style={{ color: '#8899BB' }}>Nenhuma flag cadastrada. Execute a migration 010.</p>
        )}
      </div>
      ...
      {hasMinRole(session, 'admin') && (
        ...
        >
          <h2 className="text-sm font-bold mb-4" style={{ color: '#fff' }}>Criar nova flag</h2>
