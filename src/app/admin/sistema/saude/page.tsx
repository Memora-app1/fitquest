...
import { CheckCircle2, XCircle, Activity, Database, Mail, CreditCard, Bot, Key, Globe, RefreshCw } from 'lucide-react'
...
export default async function SistemaPage() {
  ...
  const checks: Check[] = [
    ...
    },
    {
      label:    'Supabase — URL',
      ok:       env.supabaseUrl,
      detail:   env.supabaseUrl ? process.env.NEXT_PUBLIC_SUPABASE_URL!.slice(0, 40) + '…' : 'NEXT_PUBLIC_SUPABASE_URL ausente',
      icon:     <Database size={14} />,
      category: 'Banco de Dados',
    },
    {
    ...
    },
    {
      ...
      ok:       env.appUrl,
      detail:   env.appUrl ? process.env.NEXT_PUBLIC_APP_URL! : 'NEXT_PUBLIC_APP_URL ausente — links de email quebrados',
      icon:     <Globe size={14} />,
      category: 'Automações',
    },
  ]
  ...
  const statusColor = status === 'healthy' ? '#00FF88' : status === 'degraded' ? '#F5C842' : '#FF4D00'
  const statusLabel = status === 'healthy' ? 'Tudo OK' : status === 'degraded' ? 'Degradado' : 'Crítico'
    ...
    <div className="p-6 max-w-3xl mx-auto space-y-5">
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <Link
          ...
          style={{ background: 'rgba(255,255,255,0.04)', color: '#8899BB', border: '1px solid rgba(255,255,255,0.08)' }}
        >
      ...
      >
        <div
          ...
          style={{ background: `rgba(${status === 'healthy' ? '0,255,136' : status === 'degraded' ? '245,200,66' : '255,77,0'},0.1)` }}
        >
          {status === 'healthy' ? '✅' : status === 'degraded' ? '⚠️' : '🔴'}
        </div>
        <div className="flex-1 min-w-0">
          <div className="text-base font-black" style={{ color: statusColor }}>{statusLabel}</div>
        ...
        return (
          <div key={category}>
            <div className="flex items-center justify-between mb-2">
              <h2 className="text-xs font-bold uppercase tracking-wider" style={{ color: '#8899BB' }}>
                {category}
              </h2>
              <span className="text-xs" style={{ color: catOk === catChecks.length ? '#00FF88' : '#F5C842' }}>
            ...
            >
              {catChecks.map((check, i) => (
                <div
                  ...
                  style={{
                    borderBottom: i < catChecks.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none',
                ...
                >
                  ...
                  <span style={{ color: check.ok ? '#8899BB' : '#8899BB', flexShrink: 0 }}>{check.icon}</span>
                  <span className="text-sm flex-1 min-w-0" style={{ color: '#fff' }}>{check.label}</span>
                  <span
                    className="text-xs text-right shrink-0 max-w-[200px] truncate"
