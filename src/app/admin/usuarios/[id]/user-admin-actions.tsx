...
import { UserX, Zap, RefreshCw, AlertTriangle, CalendarPlus, KeyRound, CreditCard, Copy, Check } from 'lucide-react'
...
export function UserAdminActions({ userId, currentSession, isSuspended, subscriptionStatus }: Props) {
  ...
  async function changePlan() {
    ...
    const plan = status === 'active' || status === 'lifetime'
  ...
  return (
    <div className="flex flex-col gap-2 shrink-0">
      ...
      {resetLink && (
        <div
          className="px-3 py-2 rounded-lg text-xs space-y-1.5"
          style={{ background: 'rgba(59,130,246,0.08)', border: '1px solid rgba(59,130,246,0.2)' }}
        >
          <div className="font-semibold" style={{ color: '#3B82F6' }}>Link de reset (válido por 24h):</div>
          <div className="flex items-center gap-2">
            <span className="truncate text-[10px]" style={{ color: '#8899BB' }}>{resetLink}</span>
            <button onClick={copyLink} className="shrink-0 hover:opacity-70 transition-opacity">
              {copied ? <Check size={12} style={{ color: '#00FF88' }} /> : <Copy size={12} style={{ color: '#3B82F6' }} />}
            </button>
          </div>
        </div>
      )}
