  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6 p-6">
        ...
        {(activeSuspensions.data ?? []).length === 0 && (
          ...
          >
            ...
            <p className="text-sm font-semibold" style={{ color: '#fff' }}>
              Nenhum usuário suspenso no momento
            </p>
          </div>
        )}
        {(activeSuspensions.data ?? []).map((s) => {
          const profile = Array.isArray(s.profiles)
            ? (s.profiles[0] as { name: string } | undefined)
            : (s.profiles as unknown as { name: string } | null);
          ...
          return (
            ...
            >
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <form
                  action={`/api/admin/users/${s.user_id}/actions`}
                  method="POST"
                  className="shrink-0"
                >
      ...
      {(recentLifted.data ?? []).length > 0 && (
        <div>
          <h2 className="mb-3 text-sm font-bold" style={{ color: '#8899BB' }}>
            Levantadas Recentemente
          </h2>
          <div className="space-y-2">
              ...
              return (
                ...
                >
                  <div>
                    ...
                    <span className="ml-2" style={{ color: '#8899BB' }}>
                      {s.reason}
                    </span>
                  </div>
