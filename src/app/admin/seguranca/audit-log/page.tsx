...
export default async function AuditLogPage({
  ...
  let query = db
    .from('audit_logs')
    .select(
      'id, admin_id, admin_role, action, target_type, target_id, payload, ip_address, created_at',
      { count: 'exact' }
    )
  ...
  if (adminIds.length > 0) {
    const { data: profiles } = await db.from('profiles').select('id, name').in('id', adminIds);
  ...
  const { data: distinctActions } = await db.from('audit_logs').select('action').limit(100);
            ...
            {(logs ?? []).map((log, i) => {
              ...
              return (
                <tr
                  ...
                  style={{
                    borderBottom:
                      i < (logs?.length ?? 0) - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none',
                  }}
                >
                  <td className="whitespace-nowrap px-4 py-3" style={{ color: '#8899BB' }}>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1.5">
                      <span>{icon}</span>
                      <code className="font-mono" style={{ color: '#fff' }}>
                        {log.action}
                      </code>
                    </div>
                  </td>
                  <td className="px-4 py-3" style={{ color: '#8899BB' }}>
                    ...
                    {log.payload && (
                      <details className="mt-1">
                        <summary className="cursor-pointer" style={{ color: '#8899BB' }}>
                          payload
                        </summary>
                        <pre
                          className="mt-1 max-w-xs overflow-auto text-[10px]"
                          style={{ color: '#8899BB' }}
                        >
