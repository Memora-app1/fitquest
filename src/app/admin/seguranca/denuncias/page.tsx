...
export default async function ReportsPage({
  ...
  const { data: reports, count } = await db
    .from('user_reports')
    .select('id, reason, description, status, created_at, reporter_id, reported_id', { count: 'exact' })
    .eq('status', status)
      ...
      <div className="flex gap-2 flex-wrap">
        {tabs.map(tab => (
          <a
            ...
            style={{
              background: status === tab.value ? `rgba(${hexToRgb(tab.color)},0.15)` : 'rgba(255,255,255,0.04)',
      ...
      <div className="space-y-3">
        {(reports ?? []).length === 0 && (
          <div className="rounded-xl p-8 text-center" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            <p className="text-sm font-semibold" style={{ color: '#fff' }}>Nenhuma denúncia {status === 'pending' ? 'pendente' : `com status "${status}"`}</p>
          </div>
        )}
        {(reports ?? []).map(r => {
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-2">
                    <span
                      ...
                      style={{ background: `rgba(${hexToRgb(statusColor)},0.12)`, color: statusColor }}
                    >
                      {r.status}
                    </span>
                  ...
                  <div className="text-xs space-y-1">
                    <div>
                      ...
                      <Link href={`/admin/usuarios/${r.reporter_id}`} style={{ color: '#fff' }} className="hover:text-brand-orange transition-colors">
                        {reporterName}
                      </Link>
                    </div>
                    <div>
                      <span style={{ color: '#8899BB' }}>Denunciado: </span>
                      <Link href={`/admin/usuarios/${r.reported_id}`} style={{ color: '#FF4D00', fontWeight: 600 }} className="hover:opacity-80 transition-opacity">
                        {reportedName}
                      </Link>
                    </div>
                    {r.description && (
                      <div style={{ color: '#8899BB' }}>"{r.description}"</div>
                    )}
                  </div>
                </div>
                  ...
                  <div className="flex gap-2 shrink-0">
                    ...
                    <form action={`/api/admin/reports/${r.id}`} method="POST">
                      ...
                      <button
                        type="submit"
                        className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold"
                        style={{ background: 'rgba(255,255,255,0.04)', color: '#8899BB', border: '1px solid rgba(255,255,255,0.06)' }}
                      >
