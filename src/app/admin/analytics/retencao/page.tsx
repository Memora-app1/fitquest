...
export default async function RetencaoPage() {
  ...
  const [
    totalUsersRes,
    activeD1Res, activeD7Res, activeD30Res,
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: 'DAU',     value: dau.toLocaleString('pt-BR'),  sub: 'hábito hoje',   color: '#FF4D00' },
          { label: 'WAU',     value: wau.toLocaleString('pt-BR'),  sub: 'hábito em 7d',  color: '#7C3AED' },
          { label: 'MAU',     value: mau.toLocaleString('pt-BR'),  sub: 'hábito em 30d', color: '#3B82F6' },
          { label: 'DAU/MAU', value: dauMau + '%',                 sub: 'stickiness',    color: Number(dauMau) >= 20 ? '#00FF88' : '#F5C842' },
        ].map(k => (
          <div key={k.label} className="rounded-xl p-4" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.06)' }}>
            <div className="text-[11px] uppercase tracking-wider font-semibold mb-1" style={{ color: '#8899BB' }}>{k.label}</div>
            <div className="text-2xl font-black" style={{ color: k.color }}>{k.value}</div>
            <div className="text-[11px]" style={{ color: '#8899BB' }}>{k.sub}</div>
          </div>
        ))}
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-5">
        ...
        <div className="rounded-xl p-5" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.06)' }}>
          <h2 className="text-sm font-bold mb-1" style={{ color: '#fff' }}>Retenção Cohort 30d</h2>
          ...
          {cohort30Total === 0 ? (
            <p className="text-xs" style={{ color: '#8899BB' }}>Dados insuficientes.</p>
          ) : (
            <>
              <div className="text-4xl font-black mb-1" style={{ color: Number(cohort30Pct) >= 40 ? '#00FF88' : Number(cohort30Pct) >= 20 ? '#F5C842' : '#FF4D00' }}>
                {cohort30Pct}%
              </div>
              <p className="text-xs mb-3" style={{ color: '#8899BB' }}>
                {cohort30Active.toLocaleString('pt-BR')} de {cohort30Total.toLocaleString('pt-BR')} ativos
              </p>
              <div className="h-2 rounded-full" style={{ background: 'rgba(255,255,255,0.06)' }}>
                <div className="h-full rounded-full" style={{ width: `${cohort30Pct}%`, background: Number(cohort30Pct) >= 40 ? '#00FF88' : Number(cohort30Pct) >= 20 ? '#F5C842' : '#FF4D00' }} />
              </div>
              <div className="flex justify-between text-[10px] mt-1" style={{ color: '#8899BB' }}>
                <span>0%</span>
        ...
        {/* Churn / Recovery */}
        <div className="rounded-xl p-5" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.06)' }}>
          <h2 className="text-sm font-bold mb-4" style={{ color: '#fff' }}>Churn & Recovery</h2>
      ...
      <div className="rounded-xl p-5" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.06)' }}>
        <h2 className="text-sm font-bold mb-4" style={{ color: '#fff' }}>Distribuição de Streaks</h2>
        {streakTotal === 0 ? (
          <p className="text-xs" style={{ color: '#8899BB' }}>Sem dados de streak.</p>
        ) : (
          <div className="space-y-3">
            {streakRows.map(row => {
              ...
              return (
                <div key={row.label}>
                  <div className="flex items-center justify-between text-xs mb-1">
                    ...
                    <span style={{ color: row.color }}>{row.value.toLocaleString('pt-BR')} ({pct.toFixed(1)}%)</span>
                  </div>
                  <div className="h-2 rounded-full" style={{ background: 'rgba(255,255,255,0.06)' }}>
