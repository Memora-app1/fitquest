...
async function getAnalyticsData() {
  ...
  const [snapshotsRes, subscriptionBreakdownRes, topHabitsRes, xpBySourceRes] = await Promise.all([
    db.from('metrics_daily')
      .select('date, dau, new_users, total_users, habits_logged, xp_granted, active_users, trial_users')
  ...
  for (const log of (topHabitsRes.data ?? [])) {
    const h = Array.isArray(log.habits) ? log.habits[0] as { name: string; icon: string } | undefined : log.habits as { name: string; icon: string } | null
  ...
  const avgDau7 = last7.length > 0
    ? Math.round(last7.reduce((s, d) => s + (d.dau ?? 0), 0) / last7.length)
    : 0
  ...
  return { snapshots, breakdown, topHabits, maxHabitCount, xpBySource, avgDau7, totalNewLast30, totalXpLast30, totalHabitsLast30, last30days }
}
...
export default async function AnalyticsPage() {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-black" style={{ color: '#fff' }}>Analytics</h1>
        <p className="text-sm mt-0.5" style={{ color: '#8899BB' }}>Últimos 30 dias · dados dos snapshots diários</p>
      </div>

      {/* KPIs rápidos */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: 'DAU médio (7d)', value: d.avgDau7.toLocaleString('pt-BR'), icon: <Users size={14} />, color: '#00FF88' },
          { label: 'Novos usuários (30d)', value: d.totalNewLast30.toLocaleString('pt-BR'), icon: <TrendingUp size={14} />, color: '#7C3AED' },
          { label: 'Hábitos registrados (30d)', value: d.totalHabitsLast30.toLocaleString('pt-BR'), icon: <Flame size={14} />, color: '#FF4D00' },
          { label: 'XP gerado (30d)', value: d.totalXpLast30.toLocaleString('pt-BR'), icon: <Zap size={14} />, color: '#F5C842' },
        ].map(k => (
          <div key={k.label} className="rounded-xl p-4" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            <div className="text-2xl font-black" style={{ color: '#fff' }}>{k.value}</div>
          </div>
        ))}
      </div>
      ...
      <div className="rounded-xl p-5" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.06)' }}>
        <h2 className="text-sm font-bold mb-4" style={{ color: '#fff' }}>DAU — Usuários ativos diários (30d)</h2>
        {d.snapshots.length === 0 ? (
          <p className="text-xs" style={{ color: '#8899BB' }}>
            Nenhum snapshot ainda. Execute o cron <code className="px-1 py-0.5 rounded" style={{ background: 'rgba(255,255,255,0.06)' }}>/api/cron/metrics-snapshot</code> para gerar dados.
          </p>
        ) : (
            ...
            {d.snapshots.map((s, i) => {
              ...
              return (
                <div key={s.date} className="flex-1 flex flex-col items-center gap-1 group relative">
                  ...
                  />
                  {(i % 5 === 0 || isToday) && (
                    <span className="text-[9px] rotate-45 origin-left" style={{ color: '#8899BB' }}>
      ...
      <div className="grid md:grid-cols-2 gap-5">
        ...
        <div className="rounded-xl p-5" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.06)' }}>
          <h2 className="text-sm font-bold mb-4" style={{ color: '#fff' }}>Novos usuários por dia</h2>
          <div className="flex items-end gap-1 h-24">
            {d.snapshots.map((s, i) => {
              const h = Math.max(2, Math.round(((s.new_users ?? 0) / maxNew) * 96))
              return (
                <div key={s.date} className="flex-1 rounded-t-sm group relative" style={{ height: `${h}px`, background: 'rgba(124,58,237,0.5)' }}>
                  <div className="absolute -top-6 left-1/2 -translate-x-1/2 bg-zinc-800 text-white text-[10px] px-1.5 py-0.5 rounded hidden group-hover:block whitespace-nowrap z-10">
                    +{s.new_users}
                  </div>
                </div>
        ...
        <div className="rounded-xl p-5" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.06)' }}>
          <h2 className="text-sm font-bold mb-4" style={{ color: '#fff' }}>Distribuição de Planos</h2>
                ...
                return (
                  <div key={status}>
                    <div className="flex items-center justify-between text-xs mb-1">
                      ...
                      <span style={{ color }}>{count.toLocaleString('pt-BR')} ({pct.toFixed(1)}%)</span>
                    </div>
        ...
        <div className="rounded-xl p-5" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <div className="space-y-2">
            {d.topHabits.length === 0 && <p className="text-xs" style={{ color: '#8899BB' }}>Sem dados ainda.</p>}
            {d.topHabits.map((h, i) => {
              const max = d.maxHabitCount
              return (
                <div key={i} className="flex items-center gap-2 text-xs">
                  <span className="w-4 text-center">{h.icon}</span>
                  <span className="w-32 truncate" style={{ color: '#fff' }}>{h.name}</span>
                  <div className="flex-1 h-1.5 rounded-full" style={{ background: 'rgba(255,255,255,0.06)' }}>
                    <div className="h-full rounded-full" style={{ width: `${(h.count / max) * 100}%`, background: '#FF4D00' }} />
                  </div>
                  <span className="w-10 text-right font-bold" style={{ color: '#FF4D00' }}>{h.count}</span>
                </div>
              )
            })}
          </div>
        ...
        <div className="rounded-xl p-5" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <div className="space-y-2">
            {Object.entries(d.xpBySource).sort(([, a], [, b]) => b - a).map(([source, xp]) => {
              ...
              return (
                <div key={source} className="flex items-center gap-2 text-xs">
                  ...
                  <div className="flex-1 h-1.5 rounded-full" style={{ background: 'rgba(255,255,255,0.06)' }}>
                    <div className="h-full rounded-full" style={{ width: `${(xp / max) * 100}%`, background: '#F5C842' }} />
                  </div>
                  <span className="w-16 text-right font-bold" style={{ color: '#F5C842' }}>{xp.toLocaleString('pt-BR')}</span>
                </div>
              )
            })}
          </div>
