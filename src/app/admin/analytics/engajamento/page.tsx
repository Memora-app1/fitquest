...
export default async function EngajamentoPage() {
  ...
  const [habitsRes, workoutsRes, tasksRes, xpSourcesRes, snapshotsRes, topHabitsRes, levelDist] =
    await Promise.all([
      ...
      db.from('workouts').select('id', { count: 'exact', head: true }).gte('started_at', last30iso),
      // Tarefas completadas nos últimos 30d
      db.from('tasks').select('id', { count: 'exact', head: true }).gte('completed_at', last30iso),
  ...
  for (const log of topHabitsRes.data ?? []) {
    ...
    const h = Array.isArray(log.habits)
      ? (log.habits[0] as { name: string; icon: string; category: string } | undefined)
      : (log.habits as { name: string; icon: string; category: string } | null);
    if (!h) continue;
    if (!habitCounts[log.habit_id])
      habitCounts[log.habit_id] = { name: h.name, icon: h.icon, count: 0 };
  ...
  const levelEntries = Array.from(
    { length: 8 },
    (_, i) => [i + 1, levelMap[i + 1] ?? 0] as [number, number]
  );
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-6">
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        {[
          {
            label: 'Hábitos logados',
            value: habitsCount.toLocaleString('pt-BR'),
            icon: <Flame size={14} />,
            color: '#FF4D00',
          },
          {
            label: 'Treinos realizados',
            value: workoutsCount.toLocaleString('pt-BR'),
            icon: <Dumbbell size={14} />,
            color: '#7C3AED',
          },
          {
            label: 'Tarefas concluídas',
            value: tasksCount.toLocaleString('pt-BR'),
            icon: <CheckSquare size={14} />,
            color: '#3B82F6',
          },
          {
            label: 'XP gerado total',
            ...
            <div className="space-y-2">
              {topHabits.map((h, i) => (
                <div key={i} className="flex items-center gap-2 text-xs">
                  ...
                  <span className="w-28 truncate" style={{ color: '#fff' }}>
                    {h.name}
                  </span>
                  <div
                  ...
                  >
                    <div
                      ...
                      style={{
                        width: `${(h.count / maxHabitCount) * 100}%`,
                        background: '#FF4D00',
                      }}
                    />
                  </div>
                  <span className="w-12 text-right font-bold" style={{ color: '#FF4D00' }}>
                    {h.count}
                  </span>
                </div>
              ))}
                ...
                return (
                  <div key={source} className="flex items-center gap-2 text-xs">
                    ...
                    >
                      <div
                        className="h-full rounded-full"
                        style={{ width: `${(xp / maxXpSource) * 100}%`, background: color }}
                      />
                    </div>
                    <span className="w-8 text-right text-[10px]" style={{ color: '#8899BB' }}>
                      {pct}%
                    </span>
                    <span className="w-16 text-right font-bold" style={{ color }}>
      ...
      >
        ...
        <div className="flex items-end gap-2">
          {levelEntries.map(([level, count]) => {
            ...
            return (
              <div key={level} className="group relative flex flex-1 flex-col items-center gap-1">
                <div
                  className="w-full rounded-t group-hover:opacity-90"
                  style={{ height: `${h}px`, background: color }}
                />
                <span className="text-[10px] font-bold" style={{ color }}>
                  {level}
                </span>
                <div className="absolute -top-10 left-1/2 z-10 hidden -translate-x-1/2 whitespace-nowrap rounded bg-zinc-800 px-1.5 py-1 text-center text-[10px] text-white group-hover:block">
              ...
              return (
                <div key={s.date} className="group relative flex flex-1 flex-col justify-end">
                  <div
                    ...
                    style={{
                      height: `${h}px`,
                      background: isLast ? '#FF4D00' : 'rgba(255,77,0,0.35)',
                    }}
                  />
