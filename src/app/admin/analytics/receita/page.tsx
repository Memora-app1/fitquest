...
export default async function ReceitaPage() {
  ...
  const [activeSubsRes, lifetimeRes, trialRes, expiredRes, cancelledRes, snapshotsRes] = await Promise.all([
  ...
  const trialExpiring = (trialRes.data ?? []).filter(t => t.trial_end && t.trial_end <= in7days).length
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          <div key={k.label} className="rounded-xl p-4" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.06)' }}>
            <div className="text-[11px] uppercase tracking-wider font-semibold mb-2" style={{ color: '#8899BB' }}>{k.label}</div>
      ...
      <div className="grid md:grid-cols-2 gap-5">
        ...
        <div className="rounded-xl p-5" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.06)' }}>
          <h2 className="text-sm font-bold mb-4" style={{ color: '#fff' }}>Distribuição de Planos Ativos</h2>
          {planDistribution.length === 0 ? (
            <p className="text-xs" style={{ color: '#8899BB' }}>Nenhum plano ativo ainda.</p>
          ) : (
            <div className="space-y-3">
              {planDistribution.map(p => {
                const colors: Record<string, string> = { monthly: '#3B82F6', annual: '#7C3AED', lifetime: '#F5C842' }
                  ...
                  <div key={p.plan}>
                    <div className="flex items-center justify-between text-xs mb-1">
                      ...
                      <div className="flex items-center gap-2">
                        <span style={{ color: '#8899BB' }}>R$ {(p.monthly / 100).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}/mês</span>
                        <span style={{ color }}>{p.count} ({p.pct}%)</span>
                      </div>
                    </div>
        ...
        {/* Funil & alertas */}
        <div className="rounded-xl p-5" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.06)' }}>
          <h2 className="text-sm font-bold mb-4" style={{ color: '#fff' }}>Funil & Alertas</h2>
          <div className="space-y-3">
            {[
              { label: 'Em Trial',                  value: trialCount.toLocaleString('pt-BR'),      color: '#3B82F6', sub: 'potencial de conversão' },
              { label: 'Trial expirando em 7d',      value: trialExpiring.toLocaleString('pt-BR'),   color: '#F5C842', sub: 'urgente enviar e-mail', alert: trialExpiring > 0 },
              { label: 'Cancelados (acesso ativo)', value: cancelledCount.toLocaleString('pt-BR'),  color: '#8899BB', sub: 'aguardando fim do período' },
              { label: 'Expirados (30d)',            value: expiredCount.toLocaleString('pt-BR'),    color: '#FF4D00', sub: 'churned' },
            ].map(r => (
              <div key={r.label} className="flex items-center justify-between p-3 rounded-lg" style={{ background: 'rgba(255,255,255,0.03)' }}>
                <div className="flex items-center gap-2">
                  {r.alert && <AlertTriangle size={13} style={{ color: '#F5C842' }} />}
                  <div>
                    <div className="text-xs font-semibold" style={{ color: '#fff' }}>{r.label}</div>
                    <div className="text-[10px]" style={{ color: '#8899BB' }}>{r.sub}</div>
                  </div>
                </div>
                <span className="text-base font-black" style={{ color: r.color }}>{r.value}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {(snapshotsRes.data ?? []).some(s => s.mrr_cents > 0) && (
        <div className="rounded-xl p-5" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.06)' }}>
          <h2 className="text-sm font-bold mb-4" style={{ color: '#fff' }}>MRR Histórico (snapshots)</h2>
              ...
              return (
                <div key={s.date} className="flex-1 group relative flex flex-col justify-end">
                  <div
                    ...
                    style={{ height: `${h}px`, background: isLast ? '#00FF88' : 'rgba(0,255,136,0.3)' }}
                  />
                  <div className="absolute -top-7 left-1/2 -translate-x-1/2 bg-zinc-800 text-white text-[10px] px-1.5 py-0.5 rounded hidden group-hover:block whitespace-nowrap z-10">
