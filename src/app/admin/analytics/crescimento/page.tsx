...
export default async function CrescimentoPage() {
  ...
  const wowPct = prevWeekTotal > 0 ? ((lastWeekTotal - prevWeekTotal) / prevWeekTotal * 100).toFixed(1) : null
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="rounded-xl p-5" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.06)' }}>
        <h2 className="text-sm font-bold mb-4" style={{ color: '#fff' }}>Novos usuários por dia (90d)</h2>
        {noData ? (
          <p className="text-xs" style={{ color: '#8899BB' }}>Aguardando snapshots do cron <code className="px-1 rounded" style={{ background: 'rgba(255,255,255,0.06)' }}>/api/cron/metrics-snapshot</code>.</p>
        ) : (
              ...
              return (
                <div key={s.date} className="flex-1 group relative flex flex-col justify-end">
                  <div
                    ...
                    style={{ height: `${h}px`, background: isLast ? '#00FF88' : 'rgba(0,255,136,0.35)' }}
                  />
                  <div className="absolute -top-6 left-1/2 -translate-x-1/2 bg-zinc-800 text-white text-[10px] px-1.5 py-0.5 rounded hidden group-hover:block whitespace-nowrap z-10">
                    +{s.new_users} · {new Date(s.date).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' })}
                  </div>
                </div>
              )
            })}
      ...
      {/* Gráfico semanal */}
      <div className="rounded-xl p-5" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.06)' }}>
        <h2 className="text-sm font-bold mb-4" style={{ color: '#fff' }}>Novos usuários por semana (12 semanas)</h2>
              ...
              return (
                <div key={week} className="flex-1 flex flex-col items-center gap-1 group relative">
                  ...
                  <span className="text-[9px]" style={{ color: '#8899BB' }}>
                    {new Date(week).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' })}
                  </span>
                  <div className="absolute -top-6 left-1/2 -translate-x-1/2 bg-zinc-800 text-white text-[10px] px-1.5 py-0.5 rounded hidden group-hover:block whitespace-nowrap z-10">
                    +{count} na semana
                  </div>
      ...
      {!noData && (
        <div className="rounded-xl p-5" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.06)' }}>
          <h2 className="text-sm font-bold mb-4" style={{ color: '#fff' }}>Evolução da base total</h2>
            ...
            {snapshots.map((s, i) => {
              ...
              return (
                <div key={s.date} className="flex-1 group relative flex flex-col justify-end">
                  <div
                    className="w-full rounded-t-sm"
                    style={{ height: `${h}px`, background: isLast ? '#F5C842' : 'rgba(245,200,66,0.3)' }}
                  />
                  <div className="absolute -top-6 left-1/2 -translate-x-1/2 bg-zinc-800 text-white text-[10px] px-1.5 py-0.5 rounded hidden group-hover:block whitespace-nowrap z-10">
          ...
          <div className="flex justify-between mt-2 text-[10px]" style={{ color: '#8899BB' }}>
            <span>{new Date(snapshots[0]!.date).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' })}</span>
            <span>{new Date(snapshots[snapshots.length - 1]!.date).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' })}</span>
          </div>
        </div>
      )}
    </div>
