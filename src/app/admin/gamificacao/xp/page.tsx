  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="grid md:grid-cols-2 gap-5">
        ...
        >
          <h2 className="text-sm font-bold mb-4" style={{ color: '#fff' }}>Conceder / Remover XP</h2>
          <form action="/api/admin/xp-grant" method="POST" className="space-y-3">
            <div>
              <label className="text-xs mb-1 block" style={{ color: '#8899BB' }}>ID do usuário</label>
              <input
                name="user_id"
            ...
            <div>
              <label className="text-xs mb-1 block" style={{ color: '#8899BB' }}>Motivo (obrigatório)</label>
              <input
                name="reason"
            ...
            <button
              ...
              style={{ background: 'rgba(245,200,66,0.1)', color: '#F5C842', border: '1px solid rgba(245,200,66,0.25)' }}
            >
          ...
          <div className="mt-5 pt-4 border-t" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
            <p className="text-xs mb-3 font-semibold" style={{ color: '#8899BB' }}>Top usuários — clique para copiar ID</p>
            <div className="space-y-1.5">
              {(topUsers ?? []).map(u => (
                <button
                  ...
                  className="w-full flex items-center justify-between px-3 py-2 rounded-lg text-xs transition-all hover:bg-white/5 cursor-pointer"
                  style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.04)' }}
                >
                  <span style={{ color: '#fff' }}>{u.name}</span>
                  <div className="flex items-center gap-2">
                    <span style={{ color: '#8899BB' }}>Nv {u.level}</span>
                    <span style={{ color: '#F5C842' }}>⚡{(u.xp_total ?? 0).toLocaleString('pt-BR')}</span>
                  </div>
                </button>
              ))}
            </div>
        ...
        >
          ...
          {(recentGrants ?? []).length === 0 && (
            <p className="text-xs" style={{ color: '#8899BB' }}>Nenhuma concessão manual ainda.</p>
          )}
          <div className="space-y-2">
            {(recentGrants ?? []).map(g => {
              const profile = Array.isArray(g.profiles) ? (g.profiles[0] as { name: string } | undefined) : (g.profiles as unknown as { name: string } | null)
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span
                      ...
                      style={{ color: isGrant ? '#00FF88' : '#FF4D00' }}
                    >
                      {isGrant ? '+' : ''}{g.amount} XP
                    </span>
                  </div>
                  <div className="truncate" style={{ color: '#8899BB' }}>{g.reason}</div>
                  <div className="mt-1 flex items-center justify-between">
                    <span style={{ color: '#8899BB' }}>Total após: {(g.xp_total_after ?? 0).toLocaleString('pt-BR')}</span>
                    <span style={{ color: '#8899BB' }}>{new Date(g.created_at).toLocaleDateString('pt-BR')}</span>
                  </div>
                </div>
              )
            })}
