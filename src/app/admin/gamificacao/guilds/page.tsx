...
export default async function GuildsPage({
  ...
  let query = db
    .from('guilds')
    .select('id, name, tag, avatar_emoji, xp_total, weekly_xp, is_public, created_at, max_members, invite_code, guild_members(count)')
          ...
          <thead>
            <tr style={{ background: 'rgba(255,255,255,0.03)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
              {['Guild', 'Tag', 'Membros', 'XP Total', 'XP Semanal', 'Tipo', 'Criada'].map(h => (
                <th key={h} className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider" style={{ color: '#8899BB' }}>
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {(guilds ?? []).map((g, i) => {
              ...
              return (
                <tr
                  key={g.id}
                  style={{ borderBottom: i < (guilds?.length ?? 0) - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none' }}
                >
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <span className="text-lg">{g.avatar_emoji}</span>
                      <span className="font-medium" style={{ color: '#fff' }}>{g.name}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <code
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1 text-xs font-bold" style={{ color: '#F5C842' }}>
            ...
            {(guilds ?? []).length === 0 && (
              <tr>
                <td colSpan={7} className="px-4 py-8 text-center text-sm" style={{ color: '#8899BB' }}>
