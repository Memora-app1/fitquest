...
export async function POST(request: NextRequest) {
  ...
  if (!parsed.success) {
    return NextResponse.redirect(new URL('/admin/operacoes/notificacoes?error=invalid', request.url))
  }
  ...
  const { data: campaign } = await db.from('broadcast_campaigns').insert({
    ...
    scheduled_for:  parsed.data.scheduled_for
      ...
      : isSendNow ? new Date().toISOString() : null,
