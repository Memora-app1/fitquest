...
export async function GET() {
  ...
  const isPaid = ['active', 'lifetime'].includes(
    (subscription.data?.subscription_status as string) ?? ''
  );
  ...
  const totalDays = Math.ceil(
    (endDate.getTime() - new Date(season.start_date as string).getTime()) / 86400000
  );
...
export async function POST(req: NextRequest) {
  ...
  if (seasonXp < tier.season_xp_required) {
    return NextResponse.json(
      { error: 'xp_insufficient', xp_needed: tier.season_xp_required - seasonXp },
      { status: 400 }
    );
  }
  ...
  if (!tier.free) {
    ...
    const isPaid = ['active', 'lifetime'].includes(
      (profileSub?.subscription_status as string) ?? ''
    );
    if (!isPaid) {
      return NextResponse.json(
        { error: 'premium_required', message: 'Este tier requer um plano ativo.' },
        { status: 403 }
      );
    }
  }
  ...
  await serviceSupabase.from('season_progress').upsert(
    {
      user_id: user.id,
      season_id: season.id,
      season_xp: seasonXp,
  ...
  if (tier.type === 'xp' && typeof tier.value === 'number') {
    await grantXP(
      user.id,
      tier.value,
      `Season Pass — Tier ${tierNum}`,
      'season',
      `season_tier_${tierNum}`
    );
    ...
    if (cosm) {
      ...
      await serviceSupabase
        .from('profiles')
        .update({
          equipped_title: typeof tier.value === 'string' ? tier.value : String(tier.value),
        })
        .eq('id', user.id);
    }
