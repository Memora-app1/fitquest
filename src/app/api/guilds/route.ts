...
export async function GET(req: NextRequest) {
  ...
  let query = supabase
    .from('guilds')
    .select(
      'id, name, tag, motto, avatar_emoji, xp_total, weekly_xp, max_members, is_public, created_at, invite_code'
    )
    .eq('is_public', true)
  ...
  const enriched = guilds.map((g) => ({
    ...
    my_role:
      myMembership != null && myMembership.guild_id === g.id ? (myMembership.role ?? null) : null,
  }));
...
export async function POST(req: NextRequest) {
  ...
  if (guildError) {
    if (guildError.code === '23505') {
      return NextResponse.json(
        { error: 'name_taken', message: 'Esse nome de guild já existe.' },
        { status: 409 }
      );
    }
