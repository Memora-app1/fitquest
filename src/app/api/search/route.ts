...
export async function GET(req: NextRequest) {
  ...
  for (const t of tasksRes.data ?? []) {
    const priority =
      t.urgent && t.important ? 'Urgente & Importante' :
      t.urgent ? 'Urgente' :
    ...
    results.push({
      ...
      type: 'task',
      title: t.title,
      subtitle: priority ?? (t.due_date ? `Prazo: ${new Date(t.due_date).toLocaleDateString('pt-BR')}` : 'Tarefa'),
      href: '/tarefas',
  ...
  for (const t of transactionsRes.data ?? []) {
    ...
    results.push({
      ...
      type: 'transaction',
      title: t.description,
      subtitle: new Date(t.transaction_date).toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' }),
      href: '/financas/transacoes',
      icon: isIncome ? '💰' : '💳',
      color: isIncome ? '#00FF88' : '#3B82F6',
      meta: `${isIncome ? '+' : '-'}R$ ${Math.abs(t.amount ?? 0).toFixed(2).replace('.', ',')}`,
    })
  }
