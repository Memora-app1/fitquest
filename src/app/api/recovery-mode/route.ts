  ...
  if (action === 'activate') {
    if (isActive) {
      return NextResponse.json(
        { error: 'already_active', message: 'Modo Silêncio já está ativo.' },
        { status: 400 }
      );
    }
      ...
      return NextResponse.json(
        {
          ...
          message: 'Você já usou o Modo Silêncio este mês. Disponível novamente no próximo mês.',
        },
        { status: 400 }
      );
    }
    ...
    if (!isActive) {
      return NextResponse.json(
        { error: 'not_active', message: 'Modo Silêncio não está ativo.' },
        { status: 400 }
      );
    }
    ...
    const xpResult = await grantXP(
      user.id,
      200,
      'Retorno do Modo Silêncio 🌟',
      'bonus',
      `recovery_${new Date().toISOString().split('T')[0]}`
    );
