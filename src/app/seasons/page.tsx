...
export default async function SeasonsPage() {
    ...
    return (
      <AppShell>
        <div className="max-w-2xl mx-auto px-4 py-16 text-center space-y-4">
          ...
          <p className="text-text-muted">A próxima temporada começa em breve. Continue acumulando XP!</p>
        </div>
      </AppShell>
    )
  }
  ...
  const progressPct = nextTier
    ? Math.round(((seasonXp - (tiers[currentTier - 1]?.season_xp_required ?? 0)) /
        (nextTier.season_xp_required - (tiers[currentTier - 1]?.season_xp_required ?? 0))) * 100)
    : 100
