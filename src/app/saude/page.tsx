...
export default async function SaudePage() {
  ...
  const sleepTrendDays = sleepLogs.map(l => ({
    date:    l.date,
    ...
    <AppShell>
      <div className="p-4 md:p-8 max-w-4xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            { icon: Droplets,   color: '#00D9FF', rgb: '0,217,255',   label: 'Média Água',   value: avgWater7d >= 1000 ? `${(avgWater7d / 1000).toFixed(1)}L` : `${avgWater7d}ml`, sub: 'últimos 7 dias' },
            { icon: Trophy,     color: '#00FF88', rgb: '0,255,136',   label: 'Meta Água',    value: `${daysGoalReached}/7`, sub: 'dias atingidos' },
            { icon: Moon,       color: '#7C3AED', rgb: '124,58,237',  label: 'Média Sono',   value: formatDuration(avgSleep7d), sub: 'últimos 7 dias' },
            { icon: TrendingUp, color: '#F5C842', rgb: '245,200,66',  label: 'Noites Boas',  value: `${daysGoodSleep}/7`, sub: 'noites com 7h+' },
          ].map((stat) => {
            ...
            return (
              ...
              >
                <div className="absolute -top-4 -right-4 w-16 h-16 rounded-full pointer-events-none blur-xl"
                ...
                <div className="relative z-10">
                  ...
                  <div className="heading-display text-2xl" style={{ color: stat.color }}>{stat.value}</div>
