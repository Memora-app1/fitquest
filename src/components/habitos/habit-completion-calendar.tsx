...
export async function HabitCompletionCalendar({ userId }: { userId: string }) {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    const completedHabitIds = new Set(
      habits
        .filter(h => logSet.has(`${h.id}::${dateStr}`))
        .map(h => h.id)
    )
  ...
  const perfectDaysMonth = pastDays.filter(d => d.completedCount === d.totalHabits && d.totalHabits > 0).length
  const activeDaysMonth  = pastDays.filter(d => d.completedCount > 0).length
  const totalLogsMonth   = logs.length
  const avgPerDay = activeDaysMonth > 0
    ? (totalLogsMonth / Math.max(pastDays.length, 1)).toFixed(1)
    : '0'
    ...
    >
      ...
      <div className="relative z-10 space-y-5">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-sm text-text-muted mt-0.5">
              {activeDaysMonth} dias ativos · {perfectDaysMonth} dia{perfectDaysMonth !== 1 ? 's' : ''} perfeito{perfectDaysMonth !== 1 ? 's' : ''}
            </p>
          </div>
        ...
        <div className="grid grid-cols-4 gap-2">
          {[
            { label: 'Dias ativos',   value: String(activeDaysMonth),  color: '#00FF88', rgb: '0,255,136'   },
            { label: 'Dias perfeitos', value: String(perfectDaysMonth), color: '#F5C842', rgb: '245,200,66'  },
            { label: 'Registros',     value: String(totalLogsMonth),   color: '#7C3AED', rgb: '124,58,237'  },
        ...
        <div className="space-y-1.5">
          ...
          <div className="grid grid-cols-7 gap-1">
            {DOW_LABELS.map(l => (
              <div key={l} className="text-center text-[9px] text-text-muted font-medium">{l}</div>
            ))}
          </div>
            ...
            return rows.map((row, ri) => (
              <div key={ri} className="grid grid-cols-7 gap-1">
                {row.map((day, di) => {
                  ...
                  const isPerfect = day.completedCount === day.totalHabits && day.totalHabits > 0 && !day.isFuture
                  const isEmpty   = day.completedCount === 0 && !day.isFuture
                  const partial   = !isPerfect && !isEmpty && !day.isFuture
                  const completionPct = day.totalHabits > 0
                  ...
                  return (
                    <div
                      ...
                      className="rounded-lg flex flex-col items-center py-1 gap-0.5"
                      style={{
                        background: bgColor,
                      ...
                      {!day.isFuture && day.totalHabits > 0 && (
                        <div className="flex flex-wrap justify-center gap-px px-0.5">
                          ...
                          {habits.length > 6 && day.completedCount > 6 && (
                            <div
                              className="rounded-full"
                              style={{ width: '4px', height: '4px', background: 'rgba(0,255,136,0.5)' }}
                            />
                          )}
                        </div>
                      )}
          ...
          <div className="space-y-1.5">
            {habitStats.map((h, i) => (
              <div key={h.id} className="flex items-center gap-2">
                ...
                <span className="text-xs text-text-muted w-28 shrink-0 truncate">{h.name}</span>
                ...
                <span className="text-[9px] text-text-muted w-12 shrink-0 text-right">
        ...
        <div className="flex items-center gap-4 text-[9px] text-text-muted flex-wrap">
          <div className="flex items-center gap-1.5">
            <div className="w-3 h-3 rounded-sm" style={{ background: 'rgba(0,255,136,0.18)', border: '1px solid rgba(0,255,136,0.4)' }} />
            <span>Todos os hábitos</span>
          </div>
          <div className="flex items-center gap-1.5">
            <div className="w-3 h-3 rounded-sm" style={{ background: 'rgba(245,200,66,0.1)', border: '1px solid rgba(245,200,66,0.2)' }} />
            <span>Parcial</span>
          </div>
          <div className="flex items-center gap-1.5">
            <div className="w-3 h-3 rounded-sm" style={{ background: 'rgba(245,200,66,0.8)' }} />
            <span>Hoje</span>
          </div>
        </div>
