...
export async function HabitTimeOfDayHeatmap({ userId }: { userId: string }) {
  ...
  const periods = [
    { label: 'Madrugada', hours: [0, 1, 2, 3, 4, 5], color: '#7C3AED', rgb: '124,58,237', emoji: '🌙' },
    { label: 'Manhã',     hours: [6, 7, 8, 9, 10, 11], color: '#F5C842', rgb: '245,200,66', emoji: '🌅' },
    { label: 'Tarde',     hours: [12, 13, 14, 15, 16, 17], color: '#FF4D00', rgb: '255,77,0', emoji: '☀️' },
    { label: 'Noite',     hours: [18, 19, 20, 21, 22, 23], color: '#00FF88', rgb: '0,255,136', emoji: '🌆' },
  ]
  ...
  const morningPct = totalLogs > 0
    ? Math.round((periodCounts[1]!.count / totalLogs) * 100)
    : 0
  const insight =
    bestPeriod.label === 'Manhã'   ? 'Você é uma pessoa matutina — ótimo para consistência!'
    : bestPeriod.label === 'Tarde' ? 'Seu pico é na tarde — bloqueie esse horário para hábitos.'
    : bestPeriod.label === 'Noite' ? 'Você fecha o dia registrando hábitos. Consistente!'
    ...
    >
      ...
      <div className="relative z-10 space-y-4">
        ...
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="text-[10px] text-text-muted">{totalLogs} registros nos últimos 30 dias</div>
            </div>
          </div>
            ...
            return (
              <div
                ...
                style={{
                  ...
                  border: isBest ? `1px solid rgba(${p.rgb},0.3)` : '1px solid rgba(255,255,255,0.05)',
                }}
              >
                ...
                <div className="text-[9px] text-text-muted truncate">{p.label}</div>
                <div
        ...
        <div
          ...
          style={{ background: `rgba(${bestPeriod.rgb},0.05)`, border: `1px solid rgba(${bestPeriod.rgb},0.12)` }}
        >
          <span className="text-base shrink-0">{bestPeriod.emoji}</span>
          <p className="text-[11px] text-text-secondary leading-snug">{insight}</p>
        </div>
      </div>
    </div>
  )
