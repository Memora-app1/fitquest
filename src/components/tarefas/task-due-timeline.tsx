...
function urgencyColor(days: number, isOverdue: boolean): { color: string; bg: string; border: string } {
  if (isOverdue)  return { color: '#EF4444', bg: 'rgba(239,68,68,0.08)',  border: 'rgba(239,68,68,0.2)' }
  if (days === 0) return { color: '#F5C842', bg: 'rgba(245,200,66,0.08)', border: 'rgba(245,200,66,0.2)' }
  if (days <= 3)  return { color: '#FF4D00', bg: 'rgba(255,77,0,0.07)',   border: 'rgba(255,77,0,0.18)' }
  if (days <= 7)  return { color: '#7C3AED', bg: 'rgba(124,58,237,0.07)', border: 'rgba(124,58,237,0.16)' }
...
export async function TaskDueTimeline({ userId }: { userId: string }) {
  ...
  return (
    <div
      ...
      style={{
        background: 'linear-gradient(135deg, rgba(245,200,66,0.05) 0%, rgba(13,24,41,0.98) 60%, rgba(124,58,237,0.04) 100%)',
      ...
      <div className="relative z-10 space-y-5">
              ...
              <div key={bucket.key} className="space-y-2">
                ...
                <div className="space-y-1.5">
                  {bucket.tasks.map(task => {
                    ...
                    const taskUrg  = urgencyColor(bucket.isOverdue ? -1 : taskDays, bucket.isOverdue)
        ...
        <div
          ...
          style={{
            ...
            border: overdueCount > 0 ? '1px solid rgba(239,68,68,0.1)' : '1px solid rgba(245,200,66,0.1)',
          }}
        >
