...
export async function TaskDueDateHeatmap({ userId }: { userId: string }) {
  ...
  const busiest = upcomingTasks.reduce<{ date: string; count: number } | null>(
    (best, _, __, arr) => {
  ...
  return (
    <div
      ...
      style={{
        background:
          'linear-gradient(135deg, rgba(124,58,237,0.06) 0%, rgba(13,24,41,0.98) 60%, rgba(239,68,68,0.04) 100%)',
      ...
      <div className="relative z-10 space-y-5">
        ...
        <div>
          ...
          <div className="mb-1 grid grid-cols-7 gap-1">
            {DOW_LABELS.map((label) => (
              <div key={label} className="py-1 text-center text-[9px] font-medium text-text-muted">
                {label}
              </div>
            ))}
          </div>
            ...
            {weeks.map((week, wi) => (
              <div key={wi} className="grid grid-cols-7 gap-1">
                {week.map((day, di) => (
                  <div
                    key={di}
                    className="relative cursor-default rounded-lg p-1.5 transition-all duration-150 hover:scale-105"
                    style={{
                      minHeight: '52px',
                    ...
                    title={
                      day.tasks.length > 0 ? day.tasks.map((t) => t.title).join('\n') : day.dateStr
                    }
                  >
                    {/* Month label (only on first day of month) */}
                    {day.monthLabel && (
                      <div className="absolute left-1 top-0.5 text-[7px] font-bold uppercase text-text-muted">
                        {day.monthLabel}
                      </div>
                    )}
                    ...
                    {day.tasks.length > 0 && (
                      <div className="mt-1 flex flex-wrap justify-center gap-0.5">
                        ...
                        {day.tasks.length > 5 && (
                          <div className="text-[7px] font-bold text-text-muted">
                            +{day.tasks.length - 5}
                          </div>
                        )}
                      </div>
                    ...
                    {day.tasks.length >= 3 && (
                      <div
                        className="absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center rounded-full text-[7px] font-black"
                        style={{
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          >
            ...
            <div className="mt-0.5 text-[9px] text-text-muted">Críticas</div>
          </div>
          <div
            className="rounded-xl p-3 text-center"
          ...
          >
            ...
            <div className="mt-0.5 text-[9px] text-text-muted">Urgentes</div>
          </div>
          <div
            className="rounded-xl p-3 text-center"
            style={{
          ...
          >
            ...
            <div className="mt-0.5 text-[9px] text-text-muted">Normais</div>
          </div>
        </div>
