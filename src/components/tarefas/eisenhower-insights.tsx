...
export async function EisenhowerInsights({ userId }: { userId: string }) {
  ...
  const quadrants: QuadrantData[] = [
    {
      ...
      completionRate:
        activeByQ.Q1! + doneTotalByQ.Q1! > 0
    ...
    },
    {
      ...
      completionRate:
        activeByQ.Q2! + doneTotalByQ.Q2! > 0
    ...
    },
    {
      ...
      completionRate:
        activeByQ.Q3! + doneTotalByQ.Q3! > 0
    ...
    },
    {
      ...
      completionRate:
        activeByQ.Q4! + doneTotalByQ.Q4! > 0
  ...
  const focusScore = totalActive > 0 ? Math.round((activeByQ.Q2! / totalActive) * 100) : null;

  // Crisis Rate: percentage of active tasks in Q1
  const crisisRate = totalActive > 0 ? Math.round((activeByQ.Q1! / totalActive) * 100) : null;

  // Delegation rate: Q3 / total
  const delegateRate = totalActive > 0 ? Math.round((activeByQ.Q3! / totalActive) * 100) : null;

  // Waste Rate: Q4 / total
  const wasteRate = totalActive > 0 ? Math.round((activeByQ.Q4! / totalActive) * 100) : null;
  ...
  return (
    <div
      ...
      style={{
        background:
          'linear-gradient(135deg, rgba(255,77,0,0.05) 0%, rgba(13,24,41,0.98) 60%, rgba(0,255,136,0.04) 100%)',
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          {totalActive > 0 && (
            <div className="text-right">
              ...
              <div className="mt-0.5 text-[10px] font-bold" style={{ color: healthColor }}>
                {healthLabel}
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-2.5 md:grid-cols-4">
            {[
              ...
              >
                ...
                <div className="mt-0.5 text-[9px] text-text-muted">{s.subtext}</div>
              </div>
            ))}
          </div>
        )}
              ...
              return (
                <div key={q.key} className="space-y-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="w-24 shrink-0 text-xs font-bold">{q.label}</span>
                    <div
                    ...
                    <div className="flex shrink-0 items-center gap-2">
                      ...
                      <span className="w-6 text-right text-[9px] text-text-muted">
                        {activePct}%
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3 pl-7 text-[9px] text-text-muted">
                    ...
                    {q.completionRate !== null && <span>· {q.completionRate}% concluídas</span>}
                    {q.avgDaysToComplete !== null && <span>· avg {q.avgDaysToComplete}d</span>}
                    {q.doneMonth > 0 && (
              ...
              {weeklyBuckets.map((w, i) => {
                ...
                return (
                  <div key={i} className="flex flex-1 flex-col items-center gap-1">
                    {total > 0 && <span className="text-[8px] text-text-muted">{total}</span>}
                    {/* Stacked bar */}
                    <div
                    ...
                    <span className="text-[8px] capitalize text-text-muted">{w.label}</span>
                  </div>
                );
              })}
            </div>
