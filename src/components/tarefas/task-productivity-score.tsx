...
export async function TaskProductivityScore({ userId }: { userId: string }) {
  ...
  const dowStats: Record<number, { completed: number; created: number; xp: number; count: number }> = {}
  ...
  const avgQ1CycleTime = q1CycleTimes.length > 0
  ...
  for (let w = 3; w >= 0; w--) {
    ...
    const wCycleTimes = wCompleted
      ...
      .map(t => Math.max(0, (new Date(t.completed_at!).getTime() - new Date(t.created_at).getTime()) / 86400000))
    const wAvgCycle = wCycleTimes.length > 0
  ...
  const trendScore = lastWeek.completed > 0
    ? Math.min(100, Math.max(0, 50 + ((thisWeek.completed - lastWeek.completed) / lastWeek.completed) * 50))
    : 50
  const cycleScore = avgCycleTime !== null
  ...
  const prodColor = prodScore >= 80 ? '#00FF88' : prodScore >= 60 ? '#F5C842' : prodScore >= 40 ? '#FF4D00' : '#EF4444'
  const prodLabel = prodScore >= 80 ? 'Alta produtividade' : prodScore >= 60 ? 'Boa cadência' : prodScore >= 40 ? 'Pode melhorar' : 'Ritmo baixo'

  // Trend vs last week
  const weekTrend = lastWeek.completed > 0
      ...
      <div className="relative z-10 space-y-5">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          <div className="text-right">
            <div className="text-3xl font-black" style={{ color: prodColor }}>{prodScore}</div>
            <div className="text-[10px] text-text-muted uppercase tracking-wider">score produtividade</div>
            <div className="flex items-center justify-end gap-1 mt-0.5">
              {weekTrend !== null && weekTrend !== 0 && (
                <>
                  ...
                  >
                    {weekTrend > 0 ? '+' : ''}{weekTrend}% vs semana passada
                  </span>
                </>
              )}
            </div>
        ...
        <div className="space-y-2">
          <div className="text-[10px] text-text-muted uppercase tracking-wider">Concluídas por Semana</div>
        ...
        <div className="space-y-1.5">
          <div className="text-[10px] text-text-muted uppercase tracking-wider">Conclusões por Dia da Semana</div>
        ...
        <div
          ...
          style={{
            ...
            border: prodScore >= 60 ? '1px solid rgba(124,58,237,0.12)' : '1px solid rgba(255,77,0,0.1)',
          }}
        >
          ...
          <div>
            ...
            <p className="text-[11px] text-text-muted mt-0.5">
              ...
              {weekTrend !== null && weekTrend > 0 && ` +${weekTrend}% vs semana passada — ótimo ritmo!`}
            </p>
          </div>
        </div>
