...
const MUSCLE_META: Record<string, {
  label: string
  emoji: string
  color: string
  rgb: string
  ...
  chest:     { label: 'Peito',      emoji: '💪', color: '#FF4D00', rgb: '255,77,0',     recoveryHours: 48, antagonist: 'back' },
  back:      { label: 'Costas',     emoji: '🔙', color: '#7C3AED', rgb: '124,58,237',   recoveryHours: 48, antagonist: 'chest' },
  legs:      { label: 'Pernas',     emoji: '🦵', color: '#00FF88', rgb: '0,255,136',    recoveryHours: 72 },
  shoulders: { label: 'Ombros',     emoji: '🎯', color: '#F5C842', rgb: '245,200,66',   recoveryHours: 48 },
  ...
  full_body: { label: 'Corpo todo', emoji: '🏋️', color: '#8B5CF6', rgb: '139,92,246',   recoveryHours: 72 },
}
...
export async function WorkoutRestDayAdvisor({ userId }: { userId: string }) {
  ...
  const statuses: MuscleStatus[] = Object.keys(MUSCLE_META).map(muscle => {
  ...
  const todayRecommendation = readyMuscles.length >= 2
    ? readyMuscles.slice(0, 3).map(s => MUSCLE_META[s.muscle]!.label).join(' + ')
  ...
  return (
    <div
      ...
      style={{
        background: 'linear-gradient(135deg, rgba(0,217,255,0.05) 0%, rgba(13,24,41,0.98) 60%, rgba(0,255,136,0.04) 100%)',
      ...
      <div className="relative z-10 space-y-5">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-sm text-text-muted mt-0.5">
              {readyCount} grupo{readyCount !== 1 ? 's' : ''} recuperado{readyCount !== 1 ? 's' : ''} · {recoveringCount} em recuperação
            </p>
          </div>

          {/* Today's suggestion */}
          <div
            className="text-right px-3 py-2 rounded-xl flex flex-col items-end gap-0.5"
            style={{
              background: readyCount >= 2 ? 'rgba(0,255,136,0.08)' : 'rgba(245,200,66,0.07)',
              border: readyCount >= 2 ? '1px solid rgba(0,255,136,0.2)' : '1px solid rgba(245,200,66,0.18)',
            }}
          >
            <div className="text-[9px] text-text-muted uppercase tracking-wider">Treinar hoje</div>
            <div
              className="text-xs font-bold max-w-[140px] text-right"
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          >
            ...
            <div className="text-[9px] text-text-muted mt-0.5">Recuperados</div>
          </div>
          <div
            className="rounded-xl p-2.5 text-center"
          ...
          >
            ...
            <div className="text-[9px] text-text-muted mt-0.5">Recuperando</div>
          </div>
          <div
            className="rounded-xl p-2.5 text-center"
          ...
          >
            ...
            <div className="text-[9px] text-text-muted mt-0.5">Monitorados</div>
          </div>
        </div>
        ...
        >
          ...
          <div>
            ...
            <p className="text-[11px] text-text-muted mt-0.5">
              Baseado nos treinos registrados nos últimos 14 dias.
              Tempos estimados: core 24h · braços 36h · peito/costas 48h · pernas 72h.
            </p>
          </div>
        </div>
