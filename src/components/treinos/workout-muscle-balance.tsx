...
export async function WorkoutMuscleBalance({ userId }: { userId: string }) {
  ...
  for (const r of rows) {
    ...
    if (!groupMap.has(mg))
      groupMap.set(mg, { sets: 0, volume: 0, sessions: new Set(), lastDate: null });
    ...
    >
      ...
      <div className="relative z-10 space-y-5">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-0.5 text-sm text-text-muted">
              {totalSets} séries · {groups.length} grupos musculares · {formatVol(totalVolume)}{' '}
              total
            </p>
          </div>
          ...
          <div className="text-right">
            ...
            <div className="mt-0.5 text-[10px] font-bold" style={{ color: balanceColor }}>
              {balanceLabel}
            </div>
          </div>
        </div>
            ...
            return (
              <div key={g.group} className="space-y-1">
                <div className="flex items-center gap-2">
                  {/* Rank */}
                  <span
                    className="w-4 shrink-0 text-center text-[10px] font-black"
                    style={{ color: i === 0 ? '#F5C842' : '#5A6B8A' }}
                  >
                    {i + 1}
                  </span>
                  {/* Emoji */}
                  <span className="w-5 shrink-0 text-sm">{g.emoji}</span>
                  {/* Label */}
                  <span className="w-20 shrink-0 text-xs font-semibold">{g.label}</span>
                  ...
                  <div className="flex shrink-0 items-center gap-2">
                    <span className="w-8 text-right text-xs font-bold" style={{ color: g.color }}>
                      {g.sets}
                    </span>
                    <span className="w-8 text-right text-[9px] text-text-muted">{sharePct}%</span>
                  </div>
                ...
                <div className="flex items-center gap-3 pl-11 text-[9px] text-text-muted">
                  <span>
                    {g.sessions} sessão{g.sessions !== 1 ? 'ões' : ''}
                  </span>
                  ...
                  {g.daysSince !== null && (
                    <span
                      style={{
                        color:
                          g.daysSince > 14 ? '#EF4444' : g.daysSince > 7 ? '#F5C842' : '#8899BB',
                      }}
                    >
                      ·{' '}
                      {g.daysSince === 0
        ...
        <div className="space-y-2">
          <div className="text-[10px] uppercase tracking-wider text-text-muted">
            Volume por grupo (últimas 8 semanas)
          </div>
            ...
            {overdone.map((g) => (
              ...
              >
                ...
                <p className="text-[11px] text-text-muted">
                  ...
                  representa {Math.round((g.sets / totalSets) * 100)}% do treino — considere
                  diversificar.
                </p>
              </div>
            ))}
        ...
        <div
          ...
          style={{
            ...
            border:
              balanceScore >= 75 ? '1px solid rgba(0,255,136,0.1)' : '1px solid rgba(255,77,0,0.1)',
          }}
        >
