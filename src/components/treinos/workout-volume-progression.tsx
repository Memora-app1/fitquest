...
export async function WorkoutVolumeProgression({ userId }: { userId: string }) {
  ...
  const sessionTrend =
    prev4Sessions > 0
      ? Math.round(((recent4Sessions - prev4Sessions) / prev4Sessions) * 100)
      : null;
      ...
      <div className="relative z-10 space-y-5">
        ...
        <div>
          ...
          <div className="flex h-32 items-end gap-1">
            {weeks.map((w, i) => {
              ...
              return (
                ...
                >
                  <div
                    className="group relative w-full rounded-t-sm transition-all duration-500"
                    style={{
          ...
          <div className="mt-2 flex flex-wrap items-center gap-4">
            <div className="flex items-center gap-1.5">
              <div className="h-3 w-3 rounded-sm" style={{ background: 'rgba(124,58,237,0.5)' }} />
        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
          ...
          >
            ...
            <div className="mt-0.5 text-[10px] text-text-muted">12 semanas</div>
          </div>
          <div
            className="rounded-xl p-3"
            style={{
          ...
          >
            ...
            <div className="text-xl font-black text-brand-gold">
              {formatVol(avgVolumePerWeek)} kg
            </div>
            <div className="mt-0.5 text-[10px] text-text-muted">por semana</div>
          </div>
          <div
            className="rounded-xl p-3"
          ...
          >
            ...
            <div className="text-xl font-black text-brand-green">
              {formatVol(bestWeek.volume)} kg
            </div>
            <div className="mt-0.5 text-[10px] text-text-muted">{bestWeek.label}</div>
          </div>
          <div
            className="rounded-xl p-3"
            style={{
              background:
                sessionTrend !== null && sessionTrend >= 0
                  ? 'rgba(0,255,136,0.06)'
                  : 'rgba(239,68,68,0.06)',
              border:
                sessionTrend !== null && sessionTrend >= 0
          ...
          >
            ...
            {sessionTrend !== null && (
              <div className="mt-0.5 text-[10px] text-text-muted">
                {sessionTrend >= 0 ? '+' : ''}
                {sessionTrend}% vs período anterior
              </div>
            )}
          </div>
        </div>
            ...
            {topMuscles.map(([muscle, vol]) => {
              ...
              return (
                <div key={muscle} className="flex items-center gap-3">
                  ...
                  <div className="shrink-0 text-[10px] text-text-muted" style={{ width: '28px' }}>
                    {pct}%
                  </div>
                </div>
              );
