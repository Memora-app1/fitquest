      ...
      <div className="relative z-10 space-y-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="text-[10px] text-text-muted">
                {workouts.length} treinos nos últimos 90 dias
              </div>
            </div>
          </div>
        ...
        <div className="flex items-end gap-2">
          {DOW_LABELS.map((label, dow) => {
            ...
            return (
              <div key={dow} className="flex flex-1 flex-col items-center gap-1.5">
                {/* Volume label */}
                <div className="flex h-4 items-end text-[9px] text-text-muted">
                  {volume > 0
                    ? volume > 1000
                ...
                <div className="flex w-full flex-col justify-end" style={{ height: '80px' }}>
                  <div
                    className="relative w-full overflow-hidden rounded-t-lg transition-all"
                    style={{
                      ...
                      background: isRest
                        ...
                        : top
                          ...
                          : isBest
                            ...
                            : 'rgba(0,255,136,0.4)',
                      boxShadow:
                        isBest && !isRest ? `0 0 10px ${top?.color ?? '#00FF88'}50` : 'none',
                ...
                {/* Count badge */}
                {count > 0 && barH <= 30 && (
                  <div className="text-[9px] font-bold" style={{ color: top?.color ?? '#00FF88' }}>
                    {count}×
                  </div>
                )}
                ...
                {top && (
                  <div
                    className="w-full truncate rounded-full px-1.5 py-0.5 text-center text-[8px] font-bold"
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          >
            <div className="text-sm font-black text-brand-green">
              {DOW_LABELS_FULL[mostFreqDow]}
            </div>
            <div className="mt-0.5 text-[9px] text-text-muted">dia mais ativo</div>
          </div>
          <div
            className="rounded-xl p-3 text-center"
            style={{
          ...
          >
            ...
            <div className="mt-0.5 text-[9px] text-text-muted">dias de descanso</div>
          </div>
          <div
            className="rounded-xl p-3 text-center"
            style={{
          ...
          >
            <div className="text-sm font-black text-brand-gold">
              {Math.round(
                workouts.reduce((s, w) => s + (w.total_volume_kg ?? 0), 0) / workouts.length
              )}
              kg
            </div>
            <div className="mt-0.5 text-[9px] text-text-muted">volume médio/treino</div>
          </div>
        </div>
