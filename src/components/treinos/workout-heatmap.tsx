...
export async function WorkoutHeatmap({ userId }: { userId: string }) {
  ...
  const grid: Array<Array<{ date: string; count: number; isToday: boolean; isFuture: boolean } | null>> = []
  ...
  return (
    <div
      ...
      style={{
        background: 'linear-gradient(135deg, rgba(255,77,0,0.06) 0%, rgba(13,24,41,0.98) 60%, rgba(245,200,66,0.03) 100%)',
    ...
    >
      ...
      <div className="relative z-10 space-y-5">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-sm text-text-muted mt-0.5">
              {totalWorkouts} sessão{totalWorkouts !== 1 ? 'ões' : ''} em {activeDays} dia{activeDays !== 1 ? 's' : ''}
            </p>
          </div>
        ...
        <div className="grid grid-cols-3 gap-2.5">
          {[
            { label: 'Treinos (90d)', value: String(totalWorkouts), color: '#FF4D00', rgb: '255,77,0' },
            { label: 'Dias ativos', value: String(activeDays), color: '#F5C842', rgb: '245,200,66' },
            { label: 'Dia favorito', value: DAY_LABELS[favoriteDow] ?? '–', color: '#00FF88', rgb: '0,255,136' },
          ].map(s => (
            <div
              key={s.label}
          ...
          <div className="flex gap-1 mb-1">
            ...
            <div className="flex-1 grid gap-1" style={{ gridTemplateColumns: `repeat(${numWeeks}, 1fr)` }}>
              {sparseLabels.map((label, i) => (
                <div key={i} className="text-[8px] text-text-muted truncate text-center">
                  {label ?? ''}
                </div>
              ))}
            </div>
        ...
        <div className="space-y-2">
          <div className="text-[10px] text-text-muted uppercase tracking-wider">Padrão semanal</div>
            ...
            {DAY_LONG.map((day, i) => {
              ...
              return (
                <div key={day} className="flex items-center gap-2">
                  <span className="text-[10px] text-text-muted w-12 shrink-0">{day}</span>
          ...
          <div>
            ...
            {favoriteDow >= 0 && (dowCount[favoriteDow] ?? 0) > 0 && (
              <p className="text-[11px] text-text-muted mt-0.5">
                ...
                {' '}— {dowCount[favoriteDow]} sessão{(dowCount[favoriteDow] ?? 0) !== 1 ? 'ões' : ''} em 90 dias
              </p>
            )}
          </div>
        </div>
