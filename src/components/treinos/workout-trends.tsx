...
export async function WorkoutTrends({ userId }: { userId: string }) {
  ...
  const olderAvg =
    older.filter((w) => w.volume > 0).reduce((s, w) => s + w.volume, 0) /
    Math.max(1, older.filter((w) => w.volume > 0).length);
  const newerAvg =
    newer.filter((w) => w.volume > 0).reduce((s, w) => s + w.volume, 0) /
    Math.max(1, newer.filter((w) => w.volume > 0).length);
  ...
  return (
    <div
      ...
      style={{
        background:
          'linear-gradient(135deg, rgba(255,77,0,0.07) 0%, rgba(13,24,41,0.98) 60%, rgba(245,200,66,0.04) 100%)',
