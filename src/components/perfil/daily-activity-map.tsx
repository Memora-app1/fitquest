...
export async function DailyActivityMap({ userId }: { userId: string }) {
  ...
  const taskDays    = new Set((taskDoneRes.data ?? []).map(r => (r.completed_at as string).split('T')[0]))
  const workoutDays = new Set((workoutsRes.data ?? []).map(r => (r.started_at as string).split('T')[0]))
  ...
  return (
    <div
      ...
      style={{
        background: 'linear-gradient(135deg, rgba(0,255,136,0.05) 0%, rgba(13,24,41,0.98) 60%, rgba(245,200,66,0.03) 100%)',
      ...
      <div className="relative z-10 space-y-5">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-sm text-text-muted mt-0.5">
              {activeDays} dias ativos · {perfectDays} dia{perfectDays !== 1 ? 's' : ''} perfeito{perfectDays !== 1 ? 's' : ''}
            </p>
          </div>
          {currentStreak > 0 && (
            <div
        ...
        <div className="grid grid-cols-4 gap-2">
          {[
            ...
            { label: 'Dias perfeitos', value: String(perfectDays), color: '#F5C842', rgb: '245,200,66' },
            { label: 'XP (60d)', value: totalXp60 >= 1000 ? `${(totalXp60/1000).toFixed(1)}k` : String(totalXp60), color: '#F5C842', rgb: '245,200,66' },
            { label: 'XP/dia ativo', value: String(avgDailyXp), color: '#7C3AED', rgb: '124,58,237' },
          ].map(s => (
            <div
              key={s.label}
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex gap-1">
            ...
            <div className="flex-1 grid gap-1" style={{ gridTemplateColumns: `repeat(${numWeeks}, 1fr)` }}>
              {sparseLabels.map((label, i) => (
                <div key={i} className="text-[7px] text-text-muted truncate text-center">
                  {label ?? ''}
                </div>
              ))}
            </div>
          ...
          {DOW_LABELS.map((dayLabel, dayIdx) => (
            <div key={dayLabel} className="flex items-center gap-1">
              <div className="w-7 text-[9px] text-text-muted shrink-0 text-right pr-1">
                {dayIdx % 2 === 0 ? dayLabel : ''}
              </div>
              <div className="flex-1 grid gap-1" style={{ gridTemplateColumns: `repeat(${numWeeks}, 1fr)` }}>
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        boxShadow: isToday ? '0 0 6px rgba(245,200,66,0.5)' : day.score === 3 ? '0 0 4px rgba(0,255,136,0.3)' : 'none',
                      }}
                    />
                  )
                })}
        ...
        <div className="flex items-center gap-4 text-[9px] text-text-muted flex-wrap">
          <div className="flex items-center gap-1.5">
            <div className="w-3 h-3 rounded-sm" style={{ background: 'rgba(255,77,0,0.5)', border: '1px solid rgba(255,77,0,0.3)' }} />
            <span>1 domínio</span>
          </div>
          <div className="flex items-center gap-1.5">
            <div className="w-3 h-3 rounded-sm" style={{ background: 'rgba(245,200,66,0.7)', border: '1px solid rgba(245,200,66,0.4)' }} />
            <span>2 domínios</span>
          </div>
          <div className="flex items-center gap-1.5">
            <div className="w-3 h-3 rounded-sm" style={{ background: '#00FF88', border: '1px solid rgba(0,255,136,0.5)' }} />
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            { label: 'Hábitos', count: days.filter(d => d.habits).length, color: '#00FF88', rgb: '0,255,136', emoji: '🔥' },
            { label: 'Tarefas', count: days.filter(d => d.tasks).length, color: '#7C3AED', rgb: '124,58,237', emoji: '✅' },
            { label: 'Treinos', count: days.filter(d => d.workout).length, color: '#FF4D00', rgb: '255,77,0', emoji: '💪' },
          ].map(s => {
            ...
            return (
              ...
              >
                ...
                <div className="font-black text-xl leading-none" style={{ color: s.color }}>{pct}%</div>
                <div className="text-[9px] text-text-muted mt-0.5">{s.count} de 60 dias</div>
