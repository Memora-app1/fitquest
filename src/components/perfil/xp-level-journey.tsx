...
export async function XpLevelJourney({ userId, xpTotal, currentLevel }: {
  userId: string
  xpTotal: number
  currentLevel: number
}) {
  ...
  const levelProgress = levelRange > 0
    ? Math.min(100, Math.round(((xpTotal - levelXpMin) / levelRange) * 100))
    : 100
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <div className="relative z-10 space-y-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <div className="text-xs text-text-muted mt-0.5">
                ...
                {nextLevel && ` · faltam ${xpToNext.toLocaleString('pt-BR')} XP para nível ${currentLevel + 1}`}
              </div>
            </div>
          </div>
          ...
          {nextLevel && (
            <div className="space-y-1.5">
              <div className="flex justify-between text-[10px] text-text-muted">
                <span>Nível {currentLevel} — {levelInfo.title}</span>
                <span>{levelProgress}%</span>
                <span>Nível {currentLevel + 1} — {nextLevel.title}</span>
              </div>
              ...
              <div className="text-[10px] text-text-muted text-right">
                {(xpTotal - levelXpMin).toLocaleString('pt-BR')} / {levelRange.toLocaleString('pt-BR')} XP neste nível
              </div>
            </div>
          )}
          ...
          {/* Full level roadmap */}
          <div className="space-y-1.5">
            <div className="text-[10px] text-text-muted uppercase tracking-wider">Roadmap completo</div>
            <div className="flex gap-1">
              {LEVELS.filter(l => l.level <= 8).map(l => {
                ...
                return (
                  ...
                  >
                    <div
                      className="w-full h-6 rounded-lg flex items-center justify-center text-sm"
                      style={{
                        background: isDone
                    ...
                    >
                      <span className={isDone ? 'opacity-100' : isCurrent ? 'opacity-100' : 'opacity-30'}>
                        {l.emoji}
                      </span>
                    </div>
                    <span
                      ...
                      style={{
                        color: isCurrent ? '#F5C842' : isDone ? 'rgba(245,200,66,0.6)' : 'rgba(136,153,187,0.4)',
                      }}
                    >
                      {l.level}
                    </span>
      ...
      {total30d > 0 && (
        ...
        >
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="ml-auto font-black text-sm" style={{ color: '#00FF88' }}>
          ...
          <div className="mt-3 flex gap-0.5 h-2 rounded-full overflow-hidden">
            {Object.entries(sourceTotals)
              ...
              .map(([src, amount]) => {
                ...
                const pct  = Math.round((amount / total30d) * 100)
                return (
                  <div key={src} style={{ width: `${pct}%`, background: meta.color, opacity: 0.75 }} />
                )
              })}
          </div>
        </div>
        ...
        >
          <div className="flex items-center gap-2 mb-4">
            ...
            {monthTrend !== null && (
              ...
              >
                {monthTrend >= 0 ? '+' : ''}{monthTrend}% este mês
              </span>
            )}
          </div>
              ...
              return (
                <div key={m.key} className="flex-1 flex flex-col items-center gap-1">
                  <span
                    ...
                    style={{ color: isCurr ? '#7C3AED' : m.total > 0 ? '#5A6B8A' : 'rgba(90,107,138,0.3)' }}
                  >
                  ...
                  <span
                    className="text-[9px] text-text-muted capitalize"
