...
export async function XpHistory({ userId }: { userId: string }) {
  ...
  for (let i = 29; i >= 0; i--) {
    ...
    const habit = dayTxns.filter(t => t.source_type === 'habit').reduce((s, t) => s + (t.amount ?? 0), 0)
    const task = dayTxns.filter(t => t.source_type === 'task').reduce((s, t) => s + (t.amount ?? 0), 0)
    const workout = dayTxns.filter(t => t.source_type === 'workout').reduce((s, t) => s + (t.amount ?? 0), 0)
    const other = dayTxns
      .filter(t => t.source_type !== 'habit' && t.source_type !== 'task' && t.source_type !== 'workout')
  ...
  const firstAvg = first15.filter(d => d.total > 0).reduce((s, d) => s + d.total, 0) / Math.max(1, first15.filter(d => d.total > 0).length)
  const lastAvg = last15.filter(d => d.total > 0).reduce((s, d) => s + d.total, 0) / Math.max(1, last15.filter(d => d.total > 0).length)
