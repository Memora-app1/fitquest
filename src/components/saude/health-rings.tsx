...
export async function HealthRings({ userId }: { userId: string }) {
  ...
  const [waterRes, sleepRes, workoutsRes, sleepAvgRes] = await Promise.all([
    ...
    supabase.from('sleep_logs').select('duration_hours, quality').eq('user_id', userId).eq('date', yesterday).maybeSingle(),
    supabase.from('workouts').select('started_at').eq('user_id', userId).gte('started_at', `${threeDaysAgo}T00:00:00`),
    supabase.from('sleep_logs').select('duration_hours').eq('user_id', userId).gte('date', sevenDaysAgo),
  ])
  ...
  const waterLabel = waterTotal >= 1000
  ...
  const sleepLabel = sh !== null
    ? (sm === 0 ? `${sh}h` : `${sh}h${sm}m`)
    : '—'

  // ── Recovery (mesmo cálculo do RecoveryScore) ─────────────────────
  const sleepQuality = (sleepRes.data?.quality as number | null) ?? null
  let sleepScore = sleepH !== null
    ? Math.round((Math.min(100, Math.round((sleepH / 8) * 100)) * 0.6) + ((sleepQuality ?? 3) * 20 * 0.4))
    : 50
  const waterYesterdayRes = await supabase
    .from('water_logs').select('amount_ml').eq('user_id', userId).eq('date', yesterday)
  const waterYest    = (waterYesterdayRes.data ?? []).reduce((s, r) => s + (r.amount_ml as number), 0)
  const waterScore   = waterYest > 0 ? Math.min(100, Math.round((waterYest / WATER_GOAL_ML) * 100)) : 0
  const wkLast1      = (workoutsRes.data ?? []).filter(w => w.started_at >= `${yesterday}T00:00:00`).length
  const wkLast3      = (workoutsRes.data ?? []).length
  const restScore    = wkLast1 >= 1 ? 40 : wkLast3 >= 2 ? 60 : 90
  const sleepLogs7d  = (sleepAvgRes.data ?? []).filter(l => (l.duration_hours as number | null) !== null)
  const goodNights   = sleepLogs7d.filter(l => (l.duration_hours as number) >= 7).length
  const consistScore = sleepLogs7d.length >= 3 ? Math.round((goodNights / sleepLogs7d.length) * 100) : 50
  const recoveryPct  = Math.round(sleepScore * 0.35 + waterScore * 0.25 + restScore * 0.25 + consistScore * 0.15)
  ...
  const recoveryRgb   = recoveryPct >= 75 ? '0,255,136' : recoveryPct >= 50 ? '245,200,66' : '0,217,255'

  const rings: Ring[] = [
    { label: 'Sono',     emoji: '🌙', pct: sleepPct,   color: '#7C3AED', rgb: '124,58,237', value: sleepLabel,    sub: `meta ${SLEEP_GOAL_H}h`,  r: 78 },
    { label: 'Água',     emoji: '💧', pct: waterPct,   color: '#00D9FF', rgb: '0,217,255',  value: waterLabel,    sub: 'meta 2L',                r: 58 },
    { label: 'Recovery', emoji: '⚡', pct: recoveryPct, color: recoveryColor, rgb: recoveryRgb, value: `${recoveryPct}%`, sub: recoveryLabel, r: 38 },
  ]
  ...
  return (
    <div
      ...
      style={{
        background: 'linear-gradient(135deg, rgba(124,58,237,0.07) 0%, rgba(13,24,41,0.98) 55%, rgba(0,217,255,0.05) 100%)',
          ...
          <div className="relative shrink-0" style={{ width: 176, height: 176 }}>
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-0.5">
              {noData ? (
                ...
                <>
                  <span className="text-[9px] text-text-muted uppercase tracking-widest">hoje</span>
                  <span className="heading-display text-lg leading-none" style={{ color: '#00FF88' }}>
            ...
            {rings.map((ring) => (
              <div key={ring.label} className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[9px] text-text-muted mt-0.5 flex justify-between">
                    <span>{ring.sub}</span>
                    <span>{ring.pct}%</span>
                  </div>
                </div>
