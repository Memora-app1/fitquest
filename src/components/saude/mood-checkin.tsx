  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-2">
        ...
        {value > 0 && (
          <span className="text-xs font-black ml-auto" style={{ color: moodColor(value) }}>
      ...
      <div className="flex gap-2">
        {options.map(opt => (
          <button
            ...
            disabled={disabled}
            className="flex-1 h-11 rounded-xl flex items-center justify-center text-xl transition-all hover:scale-110 disabled:opacity-60"
            style={{
...
export function MoodCheckin({ todayLog, recentLogs }: Props) {
  ...
  const overallScore = canSave
    ? Math.round(((mood + energy + (6 - stress)) / 15) * 100)
    : null
  ...
  return (
    <div
      ...
      style={{
        background: 'linear-gradient(135deg, rgba(245,200,66,0.06) 0%, rgba(13,24,41,0.98) 55%, rgba(124,58,237,0.04) 100%)',
    ...
    >
      ...
      <div className="relative z-10">
        ...
        <div className="flex items-center justify-between mb-5">
          ...
          {overallScore !== null && (
            <div className="text-right">
              <div className="text-xl font-black" style={{ color: moodColor(Math.round(overallScore / 20)) }}>
                {overallScore}
              </div>
              <div className="text-[9px] text-text-muted uppercase tracking-wider">bem-estar</div>
            </div>
          )}
        </div>
        ...
        <div className="space-y-4 mb-5">
          <ScaleRow label="Humor"   emoji="😊" options={MOOD_OPTIONS}   value={mood}   onChange={setMood}   disabled={loading} />
          <ScaleRow label="Energia" emoji="⚡" options={ENERGY_OPTIONS} value={energy} onChange={setEnergy} disabled={loading} />
          <ScaleRow label="Estresse" emoji="🧠" options={STRESS_OPTIONS} value={stress} onChange={setStress} disabled={loading} />
        </div>
        ...
        >
          {loading ? 'Salvando…' : saved ? 'Atualizar check-in' : (
            <span className="flex items-center justify-center gap-2">
              Salvar check-in
              <span className="flex items-center gap-0.5 text-brand-gold text-xs font-black">
                <Zap size={11} fill="currentColor" />+10 XP
              </span>
            </span>
          )}
        </button>
        ...
        {recentLogs.length > 0 && (
          <div className="mt-4 pt-4" style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
            <div className="text-[10px] text-text-muted uppercase tracking-wider mb-2">Últimos dias</div>
            <div className="flex gap-2 overflow-x-auto pb-1">
              {recentLogs.slice(0, 5).map(log => {
                ...
                return (
                  <div
                    key={log.date}
                    className="flex flex-col items-center gap-1 shrink-0 px-2 py-1.5 rounded-xl"
                    style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)', minWidth: 44 }}
                  >
                    <span className="text-[9px] text-text-muted">{dayLabel}</span>
                    <span className="text-base leading-none">
                      {MOOD_OPTIONS.find(o => o.v === log.mood)?.emoji ?? '😐'}
                    </span>
                    <div className="w-full h-0.5 rounded-full" style={{ background: moodColor(avg) }} />
                  </div>
                )
              })}
            </div>
