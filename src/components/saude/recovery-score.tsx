...
export async function RecoveryScore({ userId }: { userId: string }) {
  ...
  const waterScore =
    waterYesterday > 0 ? Math.min(100, Math.round((waterYesterday / WATER_GOAL_ML) * 100)) : 0;
  const waterDetail =
    waterYesterday === 0
      ? 'Sem registro ontem'
  ...
  if (sleepWith7d.length >= 3) {
    const avg =
      sleepWith7d.reduce((s, l) => s + (l.duration_hours as number), 0) / sleepWith7d.length;
  ...
  const overallScore = Math.round(
    sleepScore * 0.35 + waterScore * 0.25 + restScore * 0.25 + consistencyScore * 0.15
  );
  ...
  const stateConfig = {
    pronto: {
      ...
      color: '#00FF88',
      rgb: '0,255,136',
      advice: 'Você está recuperado. Hora de dar tudo!',
    },
    parcial: {
      label: 'MODERADO',
      emoji: '🔄',
      color: '#F5C842',
      rgb: '245,200,66',
      advice: 'Recuperação parcial. Treino leve recomendado.',
    },
    recuperar: {
      label: 'RECUPERAR',
      emoji: '🛌',
      color: '#00D9FF',
      rgb: '0,217,255',
      advice: 'Priorize sono e hidratação antes de treinar pesado.',
    },
  };
      ...
      <div className="relative z-10">
        ...
        <div className="mb-5 flex items-start justify-between gap-4">
          <div>
            ...
            {/* State badge */}
            <div
              className="mt-2 inline-flex animate-bounce-in items-center gap-1.5 rounded-xl px-3 py-1.5"
              style={{
                background: `rgba(${sc.rgb},0.15)`,
                border: `1px solid rgba(${sc.rgb},0.3)`,
              }}
          ...
          <div className="relative shrink-0">
            <svg width="88" height="88">
              ...
              <circle
                ...
                style={{
                  filter: `drop-shadow(0 0 8px ${sc.color}50)`,
