...
function formatProjectedDate(date: Date): string {
  ...
  if (diffDays <= 90) return `em ~${Math.round(diffDays / 30)} mês${Math.round(diffDays / 30) > 1 ? 'es' : ''}`
...
export async function FinanceGoalsOverview({ userId }: { userId: string }) {
  ...
  const [goalsRes, txRes] = await Promise.all([
    supabase
      .from('finance_goals')
      .select('id, title, icon, color, target_amount, current_amount, deadline, monthly_target, status, completed_at, created_at')
      .eq('user_id', userId)
  ...
  const projections: GoalProjection[] = activeGoals.map(g => {
    ...
    const projectedDate = monthsNeeded !== null
      ? addMonths(new Date(), monthsNeeded)
      : null
    ...
    return { ...g, pct, remaining, monthsNeeded, projectedDate, deadlineDays, isOnTrack, monthlyRate }
  })
    ...
    <div className="space-y-4">
        ...
        <div className="relative z-10 space-y-4">
          ...
          <div>
            <div className="flex justify-between text-xs mb-1.5">
              ...
              <span className="font-bold" style={{ color: overallPct >= 75 ? '#00FF88' : '#F5C842' }}>
                {overallPct}%
              </span>
            </div>
          ...
          {avgMonthlySavings > 0 && (
            <div className="flex items-center gap-3 pt-1">
              ...
              <span className="text-sm text-text-secondary">
                ...
                <span className="text-text-muted text-xs ml-1">(últimos 3 meses)</span>
              </span>
            </div>
          )}
        </div>
      ...
      {projections.length > 0 && (
        ...
        >
          ...
          <div className="space-y-4">
            {projections.map(p => {
              const accentColor = p.isOnTrack === false
                ? '#FF4D00'
                : p.isOnTrack === true
                ? '#00FF88'
                : p.pct >= 75
              ...
              return (
                <div key={p.id} className="space-y-2">
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {p.isOnTrack === false && (
                          <AlertTriangle size={12} style={{ color: '#FF4D00' }} className="shrink-0" />
                        )}
                        {p.isOnTrack === true && (
                          <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full shrink-0" style={{ background: 'rgba(0,255,136,0.12)', color: '#00FF88' }}>
                            ✓ No prazo
                          </span>
                        )}
                      </div>
                    ...
                    <div className="text-right shrink-0">
                      <div className="font-black text-sm" style={{ color: accentColor }}>
                        {p.pct}%
                      </div>
                      ...
                      {p.remaining === 0 && (
                        <div className="text-[10px] font-bold" style={{ color: '#00FF88' }}>Meta!</div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="relative">
                    ...
                    {p.deadline && p.projectedDate && p.monthlyRate > 0 && (() => {
                      const totalDays = (new Date(p.deadline).getTime() - new Date(p.created_at).getTime()) / 86400000
                      const elapsedDays = (Date.now() - new Date(p.created_at).getTime()) / 86400000
                      const deadlinePct = Math.min(100, Math.round((elapsedDays / totalDays) * 100))
                      return (
                        <div
                          className="absolute top-0 h-2 w-0.5 rounded"
                          style={{
                            left: `${deadlinePct}%`,
                            background: p.deadlineDays && p.deadlineDays < 0 ? '#EF4444' : '#8899BB',
                          }}
                  ...
                  {p.deadline && p.deadlineDays !== null && (
                    <div className="flex items-center gap-1.5 text-[10px]">
                      <Flag size={9} style={{ color: p.deadlineDays < 0 ? '#EF4444' : p.deadlineDays <= 30 ? '#FF4D00' : '#8899BB' }} />
                      <span style={{ color: p.deadlineDays < 0 ? '#EF4444' : 'inherit' }} className="text-text-muted">
                        {p.deadlineDays < 0
        ...
        >
          ...
          <div>
            <p className="text-sm font-semibold leading-snug">
              {completedGoals.length} meta{completedGoals.length !== 1 ? 's financeiras' : ' financeira'} alcançada{completedGoals.length !== 1 ? 's' : ''}!
            </p>
            {completedGoals[0] && (
