...
const ACCOUNT_TYPE_META: Record<
  string,
  { label: string; color: string; rgb: string; icon: string }
> = {
...
export async function NetWorthSummary({ userId }: { userId: string }) {
  ...
  const creditCardDebt = creditCards.reduce(
    (s, a) => s + Math.abs(Math.min(0, Number(a.current_balance))),
    0
  );
  ...
  const creditUtilization =
    totalCreditLimit > 0 ? Math.round((totalCreditUsed / totalCreditLimit) * 100) : null;
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-0.5 text-sm text-text-muted">
                {accounts.length} conta{accounts.length !== 1 ? 's' : ''} ativa
                {accounts.length !== 1 ? 's' : ''}
              </p>
            </div>
            ...
            <div className="space-y-2 text-right">
              <div>
                <div className="text-[10px] uppercase tracking-wider text-text-muted">Ativos</div>
                <div className="text-lg font-black" style={{ color: '#00FF88' }}>
              ...
              return (
                ...
                >
                  ...
                  <div
                    ...
                    style={{ color: isNeg ? '#EF4444' : meta.color }}
                  >
                    {formatBRL(total)}
                  </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="text-xs text-text-muted">
                  de {formatBRL(totalCreditLimit)} disponível
                </div>
              </div>
