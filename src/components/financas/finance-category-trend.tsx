...
export async function FinanceCategoryTrend({ userId }: { userId: string }) {
  ...
  const catEntries = Array.from(spendMatrix.entries())
    .map(([catId, monthMap]) => ({
      catId,
      ...
      <div className="relative z-10 space-y-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <Link
            ...
            className="text-[10px] text-text-muted transition-colors hover:text-brand-purple"
          >
            Ver tudo →
          </Link>
        </div>
        ...
        <div className="grid gap-x-2" style={{ gridTemplateColumns: '1fr 1fr 1fr 1fr 1fr 60px' }}>
          <div />
          {months.map((m) => (
            <div key={m.key} className="text-center text-[9px] font-bold uppercase text-text-muted">
              {m.label}
            </div>
          ))}
          <div className="text-right text-[9px] text-text-muted">Tendência</div>
        </div>
            ...
            return (
              <div key={entry.catId} className="space-y-2">
                ...
                <div
                  ...
                  style={{ gridTemplateColumns: '1fr 1fr 1fr 1fr 1fr 60px' }}
                >
                  <div className="pr-1 text-right text-[9px] text-text-muted">
                    {formatBRL(current)}
                  </div>
                  {months.map((m) => {
                        ...
                        >
                          <div
                            ...
                            style={{
                              ...
                              background: isCurrent ? color : `${color}50`,
        ...
        {/* Legend */}
        <div className="flex items-center gap-4 border-t border-white/5 pt-1 text-[10px] text-text-muted">
