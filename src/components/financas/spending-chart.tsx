...
function StackedBar({ data, total }: { data: CategorySpend[]; total: number }) {
  ...
  return (
    <div
      className="relative flex h-3 overflow-hidden rounded-full"
      style={{ background: 'rgba(255,255,255,0.06)' }}
    >
      {data.map((item, i) => {
...
function CategoryRow({
  ...
  return (
    <div
      className="group -mx-2.5 cursor-default rounded-xl p-2.5 transition-all duration-200"
      style={{
    ...
    >
      <div className="mb-2 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          {/* Percentage pill */}
          <div
            className="rounded-full px-1.5 py-0.5 text-[10px] font-bold transition-all"
            style={{
      ...
      <div className="space-y-3 py-10 text-center">
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs text-text-muted">
            Adicione transações de despesa com categoria para ver o breakdown aqui.
          </p>
        </div>
      </div>
    );
    ...
    <div ref={ref} className="space-y-5">
      ...
      >
        ...
        <div className="text-right">
          ...
          <div className="text-base font-black" style={{ color: '#EF4444' }}>
            {formatBRL(total)}
          </div>
        </div>
      </div>
