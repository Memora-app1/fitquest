...
export async function SpendingDowHeatmap({ userId }: { userId: string }) {
  ...
  const dowAvg = dowTotal.map((total, i) => (dowOccurrences[i] ?? 1) > 0 ? total / (dowOccurrences[i] ?? 1) : 0)
  const maxDowAvg = Math.max(...dowAvg, 1)

  // Highest and lowest spending DOW
  const highestDow = dowAvg.indexOf(Math.max(...dowAvg))
  const lowestDow = dowAvg.reduce((minIdx, val, i) =>
    dowAvg[i]! > 0 && (dowAvg[minIdx] ?? 0) === 0 ? i :
    val > 0 && val < (dowAvg[minIdx] ?? Infinity) ? i : minIdx
  , highestDow)
  ...
  const avgPerDay = rows.length > 0
    ? Math.round(totalExpense / Math.max(1,
        Math.ceil((now.getTime() - new Date(threeMonthsAgo).getTime()) / 86400000)
      ))
    : 0
      ...
      <div className="relative z-10 space-y-5">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          <div className="text-right">
            <div className="text-xs text-text-muted uppercase tracking-wider mb-1">Dia mais caro</div>
            <div className="text-xl font-black" style={{ color: '#FF4D00' }}>
              {DOW_LABELS[highestDow]}
            </div>
            <div className="text-xs text-text-muted">{formatBRL(dowAvg[highestDow] ?? 0)}/semana</div>
          </div>
        </div>
              ...
              return (
                ...
                >
                  ...
                  <div
                    ...
                    style={{ color: isHighest ? '#FF4D00' : isWeekend ? 'rgba(245,200,66,0.7)' : '#4A5568' }}
                  >
                    {label}
                  </div>
                </div>
        ...
        <div>
          ...
          <div className="overflow-x-auto">
            <div style={{ minWidth: '320px' }}>
              {/* DOW header */}
              <div className="flex mb-1" style={{ paddingLeft: '64px' }}>
                {DOW_LABELS.map((label, dow) => (
                  <div
                    key={dow}
                    className="flex-1 text-center text-[9px] text-text-muted font-medium"
                  >
                    {label}
                  </div>
                ))}
              ...
              {MONTH_WEEK_LABELS.map((weekLabel, wom) => {
                ...
                return (
                  <div key={wom} className="flex items-center gap-0 mb-1">
                    {/* Week label */}
                    <div
                      className="text-[9px] text-text-muted shrink-0 pr-2 text-right"
                      style={{ width: '64px' }}
                    >
                      {weekLabel}
                    </div>
                    ...
                    {DOW_LABELS.map((_, dow) => {
                      ...
                      return (
                        <div
                          key={dow}
                          className="flex-1 mx-0.5 rounded-sm cursor-default"
                          style={{
                            height: '28px',
                            background: heatColor(val, maxWomDow),
                            border: val > 0 ? '1px solid rgba(255,77,0,0.15)' : '1px solid transparent',
                          }}
        ...
        >
          ...
          <div>
            <p className="text-sm font-semibold leading-snug">
              {DOW_LABELS[highestDow]} é seu dia de mais gastos — {formatBRL(dowAvg[highestDow] ?? 0)} em média.
              {lowestDow !== highestDow && dowAvg[lowestDow] !== undefined && (dowAvg[lowestDow] ?? 0) > 0
