...
export async function FinanceTrends({ userId }: { userId: string }) {
  ...
  const firstHalfExpense =
    firstHalf.reduce((s, d) => s + d.expense, 0) / Math.max(1, firstHalf.length);
  const secondHalfExpense =
    secondHalf.reduce((s, d) => s + d.expense, 0) / Math.max(1, secondHalf.length);
  const trend =
    firstHalfExpense > 0
  ...
  return (
    <div
      ...
      style={{
        background:
          'linear-gradient(135deg, rgba(0,255,136,0.05) 0%, rgba(13,24,41,0.98) 60%, rgba(59,130,246,0.04) 100%)',
