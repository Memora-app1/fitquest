...
export async function NextAction({ userId }: { userId: string }) {
  ...
  const [habitsRes, habitLogsRes, workoutRes, sleepRes, profileRes] = await Promise.all([
    ...
    supabase.from('workouts').select('id').eq('user_id', userId).gte('started_at', `${today}T00:00:00`).limit(1),
  ...
  type Action = { emoji: string; title: string; sub: string; xp: number; href: string; color: string; rgb: string }
  ...
  return (
    <Link
      href={best.href}
      className="block rounded-2xl p-4 relative overflow-hidden animate-fade-in transition-all hover:scale-[1.01] active:scale-[0.99]"
      style={{
    ...
    >
      <div className="absolute -top-6 -right-6 w-28 h-28 rounded-full pointer-events-none blur-2xl"
        style={{ background: `rgba(${best.rgb},0.1)` }} />

      <div className="relative z-10 flex items-center gap-4">
        <div
          className="w-12 h-12 rounded-2xl flex items-center justify-center text-2xl shrink-0"
          style={{ background: `rgba(${best.rgb},0.12)`, border: `1px solid rgba(${best.rgb},0.25)` }}
        >
          {best.emoji}
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="font-bold text-sm truncate">{best.title}</div>
