...
export async function MonthlyRecap({ userId }: MonthlyRecapProps) {
  ...
  const bestDayLabel = bestDayDate
    ? new Date(bestDayDate + 'T12:00:00').toLocaleDateString('pt-BR', {
        day: '2-digit',
        month: 'short',
      })
    : null;
  ...
  const savingsRate =
    thisIncome > 0 ? Math.round(((thisIncome - thisExpenses) / thisIncome) * 100) : null;
      ...
      <div className="relative z-10 space-y-5 p-5 md:p-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <h2 className="text-lg font-black leading-tight">{scoreLabel} este mês</h2>
          ...
          <div className="flex items-center gap-3">
            <div className="text-right">
              ...
              <div className="mt-0.5 text-xs text-text-muted">pontos</div>
            </div>
            {/* Mini SVG ring */}
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              {xpChange !== null && <TrendBadge value={xpChange} />}
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
          ...
          <StatCard
            ...
            detail={
              perfectDaysMonth > 0
                ? `${perfectDaysMonth}d perfeito${perfectDaysMonth > 1 ? 's' : ''}`
                : undefined
            }
          />
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              {perfectDaysMonth > 0 && (
                ...
                >
                  ...
                  {perfectDaysMonth} dia{perfectDaysMonth > 1 ? 's' : ''} perfeito
                  {perfectDaysMonth > 1 ? 's' : ''}
                </div>
              )}
            </div>
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 space-y-1.5">
        ...
        <div className="text-[10px] text-text-muted">{detail ?? subValue}</div>
      </div>
    </div>
  );
}
...
function TrendBadge({ value, small }: { value: number; small?: boolean }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-0.5 rounded-full px-1.5 py-0.5 font-bold ${small ? 'text-[9px]' : 'text-xs'}`}
      style={{
        background: isNeutral
          ...
          : isPositive
            ? 'rgba(0,255,136,0.1)'
            : 'rgba(239,68,68,0.1)',
        color: isNeutral ? '#8899BB' : isPositive ? '#00FF88' : '#EF4444',
        border: isNeutral
