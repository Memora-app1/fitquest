...
export async function WorkoutPrsWidget({ userId }: { userId: string }) {
  ...
  const { data: raw } = await supabase
    .from('workout_sets')
    .select('weight_kg, reps, is_warmup, is_personal_record, created_at, exercises(id, name, muscle_group)')
    .eq('user_id', userId)
    .eq('is_warmup', false)
    .gt('weight_kg', 0)
  ...
  const bestByExercise = new Map<string, { name: string; muscle: string; weight: number; reps: number; date: string }>()
  ...
  for (const r of rows) {
    ...
    if (!existing || w > existing.weight || (w === existing.weight && Number(r.reps ?? 0) > existing.reps)) {
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              {recent_pr_count > 0 && (
                <div className="text-[10px] text-text-muted">{recent_pr_count} recordes batidos</div>
              )}
            </div>
          </div>
          <Link
            ...
            return (
              <div
                ...
                style={{
                  ...
                  border: i === 0 ? '1px solid rgba(245,200,66,0.15)' : '1px solid rgba(255,255,255,0.04)',
                }}
              >
                <div
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-xs font-bold truncate">{pr.name}</div>
