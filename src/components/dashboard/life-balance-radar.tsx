...
export async function LifeBalanceRadar({ userId }: { userId: string }) {
  ...
  const [
    ...
    supabase.from('habit_logs').select('habit_id, logged_date').eq('user_id', userId).gte('logged_date', sevenDaysAgo).lte('logged_date', todayStr),
    // Tasks completed last 7 days
    supabase.from('tasks').select('id', { count: 'exact', head: true }).eq('user_id', userId).eq('status', 'done').gte('completed_at', new Date(now.getTime() - 7 * 86400000).toISOString()),
    // Workouts last 30 days
    supabase.from('workouts').select('id', { count: 'exact', head: true }).eq('user_id', userId).gte('created_at', new Date(now.getTime() - 30 * 86400000).toISOString()),
  ...
  const scores = [
    { label: 'Hábitos',          value: Math.min(100, habitScore + (habitScore > 0 ? streakBonus : 0)),     color: '#FF4D00', rgb: '255,77,0' },
      ...
      <div className="relative z-10 space-y-5">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          <div className="text-right">
            ...
            <div className="text-[10px] text-text-muted uppercase tracking-wider">score global</div>
          </div>
        </div>
