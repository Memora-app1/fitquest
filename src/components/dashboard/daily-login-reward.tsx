      ...
      if (res.ok && !json.alreadyClaimed) {
        ...
        if (json.leveledUp && json.newLevel) {
          window.dispatchEvent(
            new CustomEvent('ascendia:levelup', { detail: { level: json.newLevel } })
          );
        }
      }
    } finally {
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
        <div
          className="relative w-full max-w-sm animate-slide-up overflow-hidden rounded-3xl"
          style={{
            background: isWeek
        ...
        >
          {/* Glow top */}
          <div
            className="pointer-events-none absolute -top-8 left-1/2 h-40 w-40 -translate-x-1/2 rounded-full blur-3xl"
          ...
          <div className="relative z-10 space-y-5 p-7 text-center">
            {/* Badge */}
            <div>
              <div
              ...
              {/* Big emoji */}
              <div className="mb-2 animate-bounce-in text-6xl">{isWeek ? '🎁' : emoji}</div>
              ...
              {DAY_EMOJIS.map((e, i) => {
                ...
                return (
                  <div key={i} className="flex flex-col items-center gap-1">
                    ...
                    <div className="text-[9px] font-bold text-text-muted">+{DAY_REWARDS[i]}</div>
                  </div>
                );
              })}
            </div>
            ...
            {/* CTA */}
            {claimed ? (
              <div className="flex items-center justify-center gap-2 py-2 font-bold text-brand-green">
                ✓ Recompensa coletada!
              </div>
            ) : (
              <button
                onClick={handleClaim}
                disabled={claiming}
                className="w-full rounded-2xl py-3.5 text-sm font-black transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-60"
                style={{
