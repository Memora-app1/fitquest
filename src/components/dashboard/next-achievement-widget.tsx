...
function estimateProgress(slug: string, stats: UserStats): { current: number; target: number } | null {
      ...
      <div className="relative z-10 space-y-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="font-bold text-sm">{bestAchievement.name}</div>
            <div className="text-[11px] text-text-muted mt-0.5 line-clamp-1">{bestAchievement.description}</div>
          </div>
          <div className="text-right shrink-0">
            ...
            >
              ...
              +{bestAchievement.xp_reward}
            </div>
            <div className="text-[10px] text-text-muted mt-0.5">XP</div>
          </div>
        </div>
        ...
        <div className="space-y-1.5">
          <div className="flex items-center justify-between text-[11px]">
            <span className="text-text-muted">
              {bestProgress.current.toLocaleString('pt-BR')} / {bestProgress.target.toLocaleString('pt-BR')}
            </span>
          ...
          <div className="text-[10px] text-text-muted text-right">{bestPct}% completo</div>
        </div>
      </div>
    </div>
  )
